/*
게시글은 post-maker.html에서 추가하는 것을 권장합니다.
animal 값은 cat, dog, small 중 하나입니다. 기존 단일 이미지 형식(image)과 캐러셀 형식(images)을 모두 지원합니다.
*/

const SITE_CONFIG = {
  "storeUrl": "https://chatgpt.com/download/",
  "instagramUrl": "https://instagram.com/damyo_zip/",
  "defaultAnimal": "cat",
  "analyticsMeasurementId": "G-JXK7DE9SSP",
  "animals": {
    "cat": {
      "label": "고양이",
      "emoji": "🐱",
      "eyebrow": "DAMYOZIPSA PROMPT ARCHIVE",
      "title": "고양이 무료 프롬프트",
      "intro": "인스타그램에서 본 고양이 이미지를 선택하고\\n프롬프트를 한 번에 복사하세요.",
      "searchPlaceholder": "고양이 프롬프트 제목이나 번호로 검색",
      "usage": "내 고양이 사진을 첨부한 뒤, 복사한 프롬프트를 이미지 생성창에 붙여넣어 사용하세요.",
      "footer": "고양이와 집사를 위한 작은 아이디어를 나눕니다.",
      "instagramUrl": "https://instagram.com/damyo_zip/",
      "chatgptUrl": "https://chatgpt.com/download/",
      "affiliateUrl": "",
      "affiliateLabel": "고양이 굿즈 보러가기"
    },
    "dog": {
      "label": "강아지",
      "emoji": "🐶",
      "eyebrow": "DOG PROMPT ARCHIVE",
      "title": "강아지 무료 프롬프트",
      "intro": "인스타그램에서 본 강아지 이미지를 선택하고\\n프롬프트를 한 번에 복사하세요.",
      "searchPlaceholder": "강아지 프롬프트 제목이나 번호로 검색",
      "usage": "내 강아지 사진을 첨부한 뒤, 복사한 프롬프트를 이미지 생성창에 붙여넣어 사용하세요.",
      "footer": "강아지와 보호자를 위한 작은 아이디어를 나눕니다.",
      "instagramUrl": "",
      "chatgptUrl": "https://chatgpt.com/download/",
      "affiliateUrl": "",
      "affiliateLabel": "강아지 굿즈 보러가기"
    },
    "small": {
      "label": "햄스터",
      "emoji": "🐹",
      "eyebrow": "HAMSTER PROMPT ARCHIVE",
      "title": "햄스터 무료 프롬프트",
      "intro": "인스타그램에서 본 햄스터 이미지를 선택하고\\n프롬프트를 한 번에 복사하세요.",
      "searchPlaceholder": "햄스터 프롬프트 검색",
      "usage": "내 햄스터 사진을 첨부한 뒤, 복사한 프롬프트를 이미지 생성창에 붙여넣어 사용하세요.",
      "footer": "작고 사랑스러운 햄스터를 위한 아이디어를 나눕니다.",
      "instagramUrl": "",
      "chatgptUrl": "https://chatgpt.com/download/",
      "affiliateUrl": "",
      "affiliateLabel": "햄스터 굿즈 보러가기"
    }
  }
};

const PROMPTS = [
  {
    "id": "P-047",
    "animal": "dog",
    "title": "두 앞발에 꼭 쥔 마음",
    "category": "이미지",
    "cover": "images/p047-01.jpg",
    "description": "우리 강아지가 빨간 하트로 봉한 작은 편지를 전해 준다면",
    "prompt": "첨부한 반려동물 사진을 주인공으로 사용해 주세요. 사진 속 반려동물과 반드시 동일한 개체처럼 보이도록 종류, 털색과 털 무늬, 얼굴형, 눈의 색과 모양, 코와 입, 귀 모양, 체형과 전체적인 인상을 최대한 그대로 유지해 주세요.\n\n따뜻한 크림색 침실의 부드러운 카펫 위에서 반려동물이 자연스럽게 두 뒷발로 앉아 상체를 세운 채 카메라를 올려다보는 사실적인 생활 사진을 만들어 주세요. 반려동물은 두 앞발로 작은 크림색 종이봉투의 아래쪽 양 모서리를 가슴 앞에 꼭 붙잡고 있어야 합니다. 봉투의 삼각형 덮개가 카메라를 향하고 중앙에는 아무 글자도 없는 단순한 무광 빨간 하트 모양 봉인 하나만 붙여 주세요. 봉투는 작은 반려동물이 무리 없이 들 수 있는 자연스러운 크기이며 얼굴을 가리지 않아야 합니다.\n\n카메라는 반려동물보다 조금 높은 위치에서 내려다보는 가까운 세로형 전신 구도입니다. 반려동물의 얼굴, 양쪽 귀, 몸통, 두 앞발과 두 뒷발이 모두 프레임 안에 자연스럽게 보여야 하며 반려동물이 화면에서 가장 크고 중요한 주인공이어야 합니다. 따뜻하고 부드러운 자연광, 얕지만 현실적인 심도, 섬세한 털과 종이 질감, 포근한 크림색 배경으로 보호자가 우연히 찍은 듯한 사랑스럽고 진솔한 분위기를 표현해 주세요.\n\n다른 동물이나 사람을 추가하지 마세요. 눈, 귀, 코, 입과 발을 중복 생성하지 말고 몸통과 관절을 왜곡하거나 비정상적으로 연결하지 마세요. 사람 손, 옷, 액세서리, 읽을 수 있거나 의미 없는 글자, 로고, 워터마크, 테두리, SNS UI를 모두 제외해 주세요. Instagram 피드에 맞는 세로 4:5 비율로 화면을 채우되 얼굴, 귀, 네 발과 하트 봉투가 잘리지 않게 구성해 주세요.",
    "images": [
      {
        "src": "images/p047-01.jpg"
      }
    ]
  },
  {
    "id": "P-046",
    "animal": "small",
    "title": "햄스터가 전하는 작은 편지",
    "category": "이미지",
    "cover": "images/p046-01.jpg",
    "description": "우리 햄스터가 빨간 하트로 봉한 조그만 편지를 건넨다면",
    "prompt": "첨부한 반려동물 사진을 주인공으로 사용해 주세요. 사진 속 반려동물과 반드시 동일한 개체처럼 보이도록 종류, 털색과 털 무늬 및 배색, 얼굴형, 눈의 색과 모양, 코와 입, 귀 모양, 볼살, 작고 통통한 체형과 전체적인 인상을 최대한 그대로 유지해 주세요.\n\n따뜻한 크림색 침실의 부드러운 카펫 위에서 반려동물이 두 뒷발로 자연스럽게 서거나 앉아 상체를 세운 채 카메라를 올려다보는 사실적인 생활 사진을 만들어 주세요. 반려동물은 두 작은 앞발로 햄스터 크기에 맞는 크림색 종이봉투의 아래쪽 양 모서리를 가슴 앞에 꼭 붙잡고 있어야 합니다. 봉투의 삼각형 덮개가 카메라를 향하고 중앙에는 아무 글자도 없는 단순한 무광 빨간 하트 모양 봉인 하나만 붙여 주세요. 봉투는 반려동물이 무리 없이 들 수 있는 아주 작은 크기이며 얼굴을 가리지 않아야 합니다.\n\n카메라는 반려동물보다 조금 높은 위치에서 내려다보는 가까운 세로형 전신 구도입니다. 반려동물의 둥근 얼굴, 양쪽 귀, 몸통, 두 앞발과 두 뒷발이 모두 프레임 안에 자연스럽게 보여야 하며 반려동물이 화면에서 가장 크고 중요한 주인공이어야 합니다. 따뜻하고 부드러운 자연광, 얕지만 현실적인 심도, 섬세한 털과 종이 질감, 포근한 크림색 배경으로 보호자가 우연히 찍은 듯한 사랑스럽고 진솔한 분위기를 표현해 주세요.\n\n다른 동물이나 사람을 추가하지 마세요. 눈, 귀, 코, 입과 발을 중복 생성하지 말고 몸통과 관절을 왜곡하거나 비정상적으로 연결하지 마세요. 사람 손, 옷, 액세서리, 읽을 수 있거나 의미 없는 글자, 로고, 워터마크, 테두리, SNS UI를 모두 제외해 주세요. Instagram 피드에 맞는 세로 4:5 비율로 화면을 채우되 얼굴, 귀, 앞발과 뒷발, 하트 봉투가 잘리지 않게 구성해 주세요.",
    "images": [
      {
        "src": "images/p046-01.jpg"
      }
    ]
  },
  {
    "id": "P-045",
    "animal": "small",
    "title": "직광에 포착된 밤산책",
    "category": "이미지",
    "cover": "images/p045-01.jpg",
    "description": "우리 햄스터가 2000년대 디카에 우연히 찍힌 밤 외출의 주인공이 된다면",
    "prompt": "첨부한 반려동물 사진을 주인공으로 사용해 주세요. 사진 속 반려동물과 반드시 동일한 개체처럼 보이도록 종류, 털색과 털 무늬 및 배색, 얼굴형, 눈의 색과 모양, 코와 입, 귀 모양, 볼살, 작고 통통한 체형과 전체적인 인상을 최대한 그대로 유지해 주세요.\n\n늦은 밤 조용한 도시 골목을 산책하다가 2000년대 초반 소형 디지털카메라의 강한 직광 플래시에 우연히 포착된 듯한 사실적인 생활 스냅을 만들어 주세요. 반려동물은 낮은 회색 보도블록 위에 자연스럽게 서서 카메라를 살짝 놀란 듯 올려다봅니다. 옆에는 반려동물보다 조금 큰 반투명 미니 우산 하나만 접힌 채 놓여 있어 작은 체구가 분명히 느껴지게 해 주세요. 보호자나 사람은 화면에 등장하지 않아야 합니다.\n\n카메라는 반려동물의 눈높이보다 조금 높은 가까운 세로형 전신 구도입니다. 둥근 얼굴, 양쪽 귀, 몸통, 두 앞발과 두 뒷발이 자연스럽게 프레임 안에 보여야 하며 반려동물이 화면에서 가장 크고 중요한 주인공이어야 합니다. 정중앙의 완벽한 화보 구도보다 순간적으로 찍힌 듯한 약간 비대칭인 구도를 사용하되 얼굴과 귀와 발은 잘리지 않게 해 주세요.\n\n정면 직광 플래시가 눈과 황금빛 크림색 털에 선명한 하이라이트를 만들고, 뒤쪽 골목은 짙은 청회색 밤빛과 약간 부족한 노출로 자연스럽게 어두워지게 해 주세요. 초기 디지털카메라 특유의 미세한 센서 노이즈, 약간 차가운 색감, 가벼운 색 번짐과 솔직한 생활 사진 질감을 표현하되 저해상도나 심한 모션 블러처럼 보이지 않게 해 주세요. 전체 분위기는 꾸민 화보보다 보호자가 한밤중에 우연히 남긴 솔직하고 장난스러운 레트로 생활 사진처럼 느껴져야 합니다. 배경에는 흐릿한 금속 셔터와 작은 둥근 조명 한두 개만 두고 읽을 수 있는 간판이나 문구는 넣지 마세요.\n\n다른 동물이나 사람을 추가하지 마세요. 눈, 귀, 코, 입, 앞발과 뒷발을 중복 생성하지 말고 몸통과 관절을 왜곡하지 마세요. 우산이나 배경 물체가 얼굴, 귀, 앞발, 발을 가리지 않게 해 주세요. 옷, 액세서리, 글자, 로고, 워터마크, 테두리, 날짜 표시, SNS UI를 모두 제외해 주세요. Instagram 피드에 맞는 세로 4:5 비율로 화면을 채우되 얼굴, 귀, 발과 미니 우산이 프레임 안에 온전히 들어오게 해 주세요.",
    "images": [
      {
        "src": "images/p045-01.jpg"
      }
    ]
  },
  {
    "id": "P-044",
    "animal": "dog",
    "title": "문틈에 또 하나의 나",
    "category": "이미지",
    "cover": "images/p044-01.jpg",
    "description": "우리 강아지가 평범한 집 사진 속 문틈에서 한 번 더 등장한다면",
    "prompt": "첨부한 강아지 사진을 주인공으로 사용해 주세요. 사진 속 반려동물과 반드시 같은 개체처럼 보이도록 순백색 털색과 자연스러운 털 무늬, 둥근 얼굴형, 크고 짙은 눈, 작은 검은 코와 짧은 주둥이, 부드럽게 처진 귀, 작은 체형과 전체적인 인상을 최대한 그대로 유지해 주세요.\n\n평범하고 아늑한 집의 거실과 복도가 이어지는 공간에서, 반려동물이 카메라 가까운 거실 바닥에 얌전히 앉아 정면을 바라보는 사실적인 스마트폰 사진을 만들어 주세요. 주인공 뒤쪽의 살짝 열린 방문 틈에는 같은 반려동물이 머리와 가슴 윗부분만 조용히 내밀고 한 번 더 등장해야 합니다. 앞쪽의 주인공과 문틈의 모습은 의도적으로 동일한 반려동물이 두 장소에 동시에 있는 초현실적인 장면이며, 다른 동물이나 제3의 반려동물은 넣지 마세요. 문틈의 반려동물도 같은 털색, 얼굴, 눈, 코, 귀가 알아볼 수 있게 표현하되 앞쪽 주인공보다 작고 배경에 자연스럽게 배치해 주세요.\n\n카메라 구도는 보호자가 거실에서 우연히 찍은 듯한 반려동물 눈높이의 세로형 미디엄 전신 구도입니다. 앞쪽 주인공이 화면에서 가장 크고 중요한 초점이어야 하며 얼굴, 양쪽 귀, 몸통과 네 발이 자연스럽게 보여야 합니다. 배경의 문간은 화면 위쪽 3분의 1 지점에 또렷하게 읽히되 얕은 심도로 살짝 부드럽게 표현해 주세요. 조명은 저녁의 따뜻한 실내등과 복도에서 들어오는 약간 차가운 빛이 섞이게 해 주세요. 배경은 정돈된 평범한 집으로 하고 소품을 과하게 늘어놓지 마세요. 전체 분위기는 무섭기보다는 처음에는 평범하지만 뒤늦게 문틈의 같은 강아지를 발견하면 웃음과 작은 소름이 함께 오는 저예산 호러 코미디 밈 사진입니다.\n\n의도된 두 모습 각각에서 눈, 귀, 코, 입과 발을 중복 생성하지 말고 정상적인 개수와 연결로 표현해 주세요. 몸통, 다리와 관절을 왜곡하지 말고 얼굴이 문이나 가구와 합쳐지지 않게 해 주세요. 사람, 다른 동물, 그림 속 동물, 인형은 넣지 마세요. 읽을 수 있거나 의미 없는 글자, 텍스트, 문패, 간판, 로고, 워터마크, 테두리, SNS UI를 모두 제외해 주세요. Instagram 피드에 맞는 세로 4:5 비율로 화면을 채우되 앞쪽 주인공의 얼굴과 귀와 발, 뒤쪽 문틈의 얼굴이 잘리지 않게 구성해 주세요.",
    "images": [
      {
        "src": "images/p044-01.jpg"
      }
    ]
  },
  {
    "id": "P-043",
    "animal": "small",
    "title": "미니 세탁방의 양말 정리",
    "category": "이미지",
    "cover": "images/p043-01.jpg",
    "description": "우리 햄스터가 조그만 세탁방의 양말 정리 담당이 된다면",
    "prompt": "첨부한 반려동물 사진을 주인공으로 사용해 주세요. 사진 속 반려동물과 반드시 동일한 개체처럼 보이도록 종류, 털색과 털 무늬 및 배색, 얼굴형, 눈의 색과 모양, 코와 입, 귀 모양, 볼살, 체형과 전체적인 인상을 최대한 그대로 유지해 주세요.\n\n포근하고 깨끗한 미니어처 셀프 세탁방에서 반려동물이 작은 빨래 정리 도우미가 된 사실적인 사진을 만들어 주세요. 반려동물은 낮은 연한 원목 접이 테이블 뒤에 자연스럽게 서서 두 앞발로 크림색 미니 양말 한 짝만 조심스럽게 들고 카메라를 바라봅니다. 한쪽에는 파스텔 천 두 장이 단정하게 접혀 담긴 작은 라탄 바구니 하나를 놓아 주세요. 뒤에는 커다란 원형 유리문이 달린 드럼 세탁기 한 대가 보여 반려동물의 작은 체구와 귀여운 크기 대비가 분명히 느껴지게 해 주세요.\n\n배경은 옅은 민트색 타일과 밝은 원목으로 꾸민 정돈된 미니 세탁방입니다. 카메라는 반려동물의 눈높이보다 아주 조금 높은 위치에서 촬영한 세로형 미디엄 전신 구도이며, 둥근 얼굴과 양쪽 귀, 몸통, 두 앞발과 두 발이 자연스럽게 확인되어야 합니다. 반려동물의 눈과 얼굴에는 또렷하게 초점을 맞추고 세탁기와 공간은 알아볼 수 있는 정도의 자연스러운 얕은 심도로 부드럽게 표현해 주세요. 창으로 들어오는 따뜻한 아침 자연광과 은은한 실내 조명이 섞인 깨끗하고 포근한 분위기, 보호자가 우연히 발견해 스마트폰으로 찍은 듯한 생활 사진 느낌으로 완성해 주세요.\n\n반려동물이 화면에서 가장 크고 중요한 주인공이어야 하며 다른 동물이나 사람은 넣지 마세요. 눈, 귀, 코, 입, 앞발과 발을 중복 생성하지 말고, 관절과 몸통을 왜곡하지 마세요. 양말은 앞발 사이에 한 짝만 두고, 테이블이나 바구니가 얼굴, 귀, 앞발, 발을 가리지 않게 해 주세요. 읽을 수 있거나 의미 없는 글자, 세탁기 표시, 가격표, 안내판, 로고, 워터마크, 테두리, SNS UI는 모두 제외해 주세요. Instagram 피드에 맞는 세로 4:5 비율로 화면을 채우되 얼굴, 귀, 발과 주요 소품이 잘리지 않게 구성해 주세요.",
    "images": [
      {
        "src": "images/p043-01.jpg"
      }
    ]
  },
  {
    "id": "P-042",
    "animal": "dog",
    "title": "창가에 앉은 작은 여행자",
    "category": "이미지",
    "cover": "images/p042-01.jpg",
    "description": "우리 강아지가 늦여름 완행열차의 작은 여행자가 된다면",
    "prompt": "첨부한 강아지 사진을 주인공으로 사용해 주세요. 사진 속 반려동물과 반드시 동일한 개체처럼 보이도록 털색, 털 무늬, 얼굴형, 눈의 색과 모양, 코와 입, 귀 모양, 체형과 전체적인 인상을 최대한 그대로 유지해 주세요.\n\n늦여름 오후의 조용한 완행열차에서 반려동물이 창가의 작은 여행자가 된 사실적인 사진을 만들어 주세요. 반려동물은 짙은 남색 패브릭 좌석 위에 자연스럽게 앉아 카메라를 살짝 올려다봅니다. 옆에는 작은 크림색 캔버스 여행 파우치 하나를 놓고, 얼굴과 귀를 가리지 않는 연한 하늘색 손수건을 목 아래에 느슨하게 둘러 주세요. 배경은 깨끗하고 차분한 열차 내부로 표현하고, 큰 창밖으로는 초록빛 논과 따뜻한 노을 하늘이 열차의 움직임을 느낄 수 있도록 부드럽게 흐려져 보이게 해 주세요.\n\n카메라는 반려동물의 눈높이에서 촬영한 세로형 미디엄 전신 구도이며, 얼굴과 양쪽 귀, 몸통과 네 발의 형태가 프레임 안에서 자연스럽게 확인되어야 합니다. 반려동물의 눈과 얼굴에는 또렷하게 초점을 맞추고 열차 내부와 창밖 풍경은 자연스러운 얕은 심도로 부드럽게 표현해 주세요. 창으로 들어오는 늦은 오후의 꿀빛 햇살이 털을 은은하게 비추고, 작은 여행을 앞둔 설렘과 평온함이 함께 느껴지는 보호자의 스마트폰 여행 사진 분위기로 완성해 주세요.\n\n반려동물이 화면에서 가장 크고 중요한 주인공이어야 하며 다른 동물이나 사람은 넣지 마세요. 눈, 귀, 코, 입을 중복 생성하지 말고, 앞발과 뒷발을 정상적인 개수와 연결로 표현하며 몸통과 관절을 왜곡하지 마세요. 여행 파우치나 손수건이 얼굴, 귀, 발을 가리지 않게 해 주세요. 읽을 수 있거나 의미 없는 글자, 좌석 번호, 안내판, 로고, 워터마크, 테두리, SNS UI는 모두 제외해 주세요. Instagram 피드에 맞는 세로 4:5 비율로 화면을 채우되 얼굴, 귀, 발과 주요 소품이 잘리지 않게 구성해 주세요.",
    "images": [
      {
        "src": "images/p042-01.jpg"
      }
    ]
  },
  {
    "id": "P-041",
    "animal": "small",
    "title": "심야 편의점 알바생",
    "category": "이미지",
    "cover": "images/p041-01.jpg",
    "description": "우리 햄스터가 조그만 심야 편의점의 야간 알바생이 된다면",
    "prompt": "첨부한 반려동물 사진을 주인공으로 사용해 주세요. 사진 속 반려동물과 반드시 동일한 개체처럼 보이도록 종류, 털색과 털 무늬 및 배색, 얼굴형, 눈의 색과 모양, 코와 입, 귀 모양, 볼살, 체형과 전체적인 인상을 최대한 그대로 유지해 주세요.\n\n늦은 밤의 작고 아늑한 미니어처 편의점에서 반려동물이 야간 알바생처럼 카운터 안쪽에 서 있는 사실적인 사진을 만들어 주세요. 반려동물 앞에는 작은 계산대와 바코드 스캐너가 있고, 한쪽 앞발은 카운터 위에 자연스럽게 올려 둔 채 카메라를 바라봅니다. 얼굴과 귀를 가리지 않는 단정한 짙은 초록색 미니 앞치마만 입혀 주세요. 카운터 위에는 손님이 고른 듯한 삼각김밥 모양의 작은 포장 하나와 따뜻한 음료 컵 하나만 놓아 주세요. 포장과 컵에는 읽을 수 있는 글자나 실제 브랜드 로고가 없어야 합니다.\n\n카메라는 반려동물의 눈높이보다 아주 조금 높은 위치에서 촬영한 세로형 미디엄 전신 구도입니다. 반려동물의 둥근 얼굴, 양쪽 귀, 몸통, 앞발과 뒷발이 자연스럽게 확인되어야 하며, 카운터와 진열대는 반려동물의 작은 체구가 귀엽게 느껴지는 미니어처 크기로 표현해 주세요. 얕은 심도로 배경의 정돈된 간식 진열대와 냉장고 불빛은 부드럽게 흐려지고, 반려동물의 눈과 얼굴에는 또렷하게 초점이 맞아야 합니다. 조명은 편의점의 따뜻한 천장 조명과 창밖의 은은한 푸른 밤빛이 섞인 자연스러운 야간 생활 사진 분위기입니다.\n\n반려동물이 화면에서 가장 크고 중요한 주인공이어야 하며 다른 동물이나 사람은 넣지 마세요. 눈, 귀, 코, 입, 앞발과 뒷발을 중복 생성하지 말고, 관절과 몸통을 왜곡하지 마세요. 계산대나 상품이 얼굴, 귀, 발을 가리지 않게 해 주세요. 읽을 수 있거나 의미 없는 글자, 가격표 문구, 간판, 로고, 워터마크, 테두리, SNS UI는 모두 제외해 주세요. Instagram 피드에 맞는 세로 4:5 비율로 화면을 채우되 얼굴, 귀, 발과 주요 소품이 잘리지 않게 구성해 주세요.",
    "images": [
      {
        "src": "images/p041-01.jpg"
      }
    ]
  },
  {
    "id": "P-040",
    "animal": "dog",
    "title": "새벽 빵집 첫 손님",
    "category": "이미지",
    "cover": "images/p040-01.jpg",
    "description": "우리 강아지가 갓 구운 빵 냄새를 따라 새벽 빵집의 첫 손님이 된다면",
    "prompt": "첨부한 강아지 사진을 주인공으로 사용해 주세요. 사진 속 반려동물과 반드시 동일한 개체처럼 보이도록 털색, 털 무늬, 얼굴형, 눈의 색과 모양, 코와 입, 귀 모양, 체형과 전체적인 인상을 최대한 그대로 유지해 주세요.\n\n문을 열기 직전의 아늑한 동네 빵집에서 반려동물이 ‘새벽 빵집의 첫 손님’이 된 사실적인 사진을 만들어 주세요. 반려동물은 낮은 원목 카운터 앞의 작은 둥근 의자에 자연스럽게 앉아 카메라를 바라봅니다. 카운터 위에는 갓 구운 크루아상 하나와 작은 식빵 하나를 담은 심플한 나무 트레이만 놓아 주세요. 반려동물에게는 얼굴과 귀를 가리지 않는 연한 베이지색 체크 손수건을 목에 느슨하게 둘러 주세요.\n\n카메라는 반려동물의 눈높이에서 촬영한 세로형 미디엄 전신 구도이며, 얼굴과 양쪽 귀, 몸통과 네 발의 형태가 프레임 안에서 자연스럽게 확인되어야 합니다. 얕은 심도로 배경의 나무 빵 진열대와 작은 오븐은 부드럽게 흐려지고, 반려동물의 얼굴과 눈에는 또렷하게 초점이 맞아야 합니다. 조명은 오븐과 펜던트 조명의 따뜻한 황금빛에 창문으로 들어오는 은은한 푸른 새벽빛이 섞이게 해 주세요. 배경은 정돈된 작은 베이커리로 표현하고 소품과 빵을 과하게 늘어놓지 마세요. 전체 분위기는 하루가 시작되기 직전의 설렘, 포근함, 갓 구운 빵의 따뜻함과 반려동물의 살짝 엉뚱한 귀여움이 느껴지는 고급 라이프스타일 화보입니다.\n\n반려동물이 화면에서 가장 크고 중요한 주인공이어야 하며 다른 동물이나 사람은 넣지 마세요. 눈, 귀, 코, 입을 중복 생성하지 말고, 앞발과 뒷발을 정상적인 개수와 연결로 표현하며 몸통과 관절을 왜곡하지 마세요. 빵이나 트레이가 얼굴, 귀, 발을 가리지 않게 해 주세요. 읽을 수 있거나 의미 없는 글자, 메뉴판 문구, 포장지 글자, 간판, 로고, 워터마크, 테두리, SNS UI는 모두 제외해 주세요. Instagram 피드에 맞는 세로 4:5 비율로 화면을 꽉 채우되 얼굴, 귀, 발과 주요 소품이 잘리지 않게 구성해 주세요.",
    "images": [
      {
        "src": "images/p040-01.jpg"
      }
    ]
  },
  {
    "id": "P-039",
    "animal": "small",
    "pinned": true,
    "title": "햄님이의 여름휴가",
    "category": "스티커",
    "cover": "images/p039-01.png",
    "images": [
      {
        "src": "images/p039-01.png",
        "title": "여름엔 카페",
        "caption": "이미지마다 프롬프트가 달라요",
        "prompt": "첨부한 반려동물 사진과 반드시 같은 반려동물처럼 보이는 여름 카페 휴식 테마의 귀여운 한국식 이모티콘 스티커 시트를 만들어 주세요.\n반려동물의 털색, 줄무늬, 얼굴형, 눈, 코, 귀, 볼살, 체형, 목걸이를 최대한 그대로 유지해 주세요.\n16개의 스티커는 모두 같은 반려동물로 보이되, 복붙처럼 똑같아 보이지 않게 표정, 포즈, 문구, 소품을 다양하게 바꿔 주세요.\n\n전체 이미지는 상업용 스티커 시트 디자인처럼 깔끔하고 완성도 높게 표현해 주세요.\n16개의 개별 스티커를 4x4 배열로 배치하고, 각 스티커는 서로 겹치지 않게 적당한 간격을 두고 배치해 주세요.\n각 스티커에는 흰색 칼선 느낌의 외곽 테두리를 분명하게 넣어 주세요.\n\n중요하게, 흰색 외곽선이 잘 보이도록 전체 시트의 바탕 배경은 흰색이 아닌 아주 연한 베이지색 단색 배경으로 표현해 주세요.\n배경은 복잡한 무늬 없이 밝고 부드러운 연베이지 톤이어야 하며, 스티커가 잘 돋보이도록 깔끔하게 처리해 주세요.\n예시 느낌: 아주 연한 크림베이지 / 부드러운 종이 느낌의 밝은 베이지.\n\n각 스티커는 모두 서로 다른 표정, 포즈, 문구, 장식, 여름 소품을 가져야 합니다.\n아래 요소들은 매번 임의로 조합을 바꿔 새로운 세트처럼 구성해 주세요.\n\n표정 후보:\n설렘, 여유, 만족, 반짝이는 눈, 졸림, 장난기, 멍함, 신남, 시원해서 행복함, 사랑스러움\n\n포즈 후보:\n정면, 고개 갸웃, 디저트 옆에 앉기, 아이스 음료 옆에 앉기, 선글라스 쓰기, 두 앞발 모으기, 테이블 느낌의 소품 앞에 앉기, 수박 빙수 옆에 앉기, 빨대 컵 옆에 기대기, 낮잠 자기\n\n문구 후보:\n카페 왔어요!, 빙수 먹어요!, 아이스가 최고!, 오늘은 여유롭게, 디저트 타임!, 시원해요~, 엄마도 한입, 휴가 중이에요, 여름 간식 최고!, 사진 찍어 주세요!, 행복한 휴식, 놀 준비 끝!, 수박 주세요!, 너무 좋아요!, 쉬는 중이에요, 달달해요\n\n장식/소품 후보:\n아이스 음료, 빙수, 수박, 아이스크림, 빨대컵, 디저트 접시, 체리, 선글라스, 반짝이, 하트, 작은 파라솔, 민트잎, 부채, 얼음 조각, 카페 트레이\n\n전체 분위기는 귀엽고 밝고 선명한 상업용 스티커 느낌으로 표현해 주세요.\n각 스티커는 감정 표현이 잘 드러나고, 카카오톡 이모티콘처럼 직관적이고 사랑스럽게 만들어 주세요.\n문구는 각 스티커 아래 또는 어울리는 위치에 짧고 잘 읽히는 한국어 텍스트로 넣어 주세요.\n텍스트 색상은 스티커마다 어울리게 다양하게 적용하되, 전체적으로 통일감 있게 구성해 주세요.\n\n주의사항:\n같은 반려동물로 반드시 통일감 있게 표현할 것\n16개 모두 서로 다른 느낌으로 만들 것\n흰 칼선 외곽이 배경과 명확히 구분되도록 할 것\n실사 배경, 손, 책상, SNS UI, 그림자 효과 없이 오직 스티커 시트만 표현할 것\n완성 이미지는 스티커 상품 제작용 시안처럼 깔끔하고 선명하게 표현할 것\n\n[필수: AR 4:5]"
      },
      {
        "src": "images/p039-02.png",
        "title": "여름엔 방콕",
        "caption": "이미지마다 프롬프트가 달라요",
        "prompt": "첨부한 반려동물 사진과 반드시 같은 반려동물처럼 보이는 여름 집콕 피서 테마의 귀여운 한국식 이모티콘 스티커 시트를 만들어 주세요.\n반려동물의 털색, 줄무늬, 얼굴형, 눈, 코, 귀, 볼살, 체형, 목걸이를 최대한 그대로 유지해 주세요.\n16개의 스티커는 모두 같은 반려동물로 보이되, 복붙처럼 똑같아 보이지 않게 표정, 포즈, 문구, 소품을 다양하게 바꿔 주세요.\n\n전체 이미지는 상업용 스티커 시트 디자인처럼 깔끔하고 완성도 높게 표현해 주세요.\n16개의 개별 스티커를 4x4 배열로 배치하고, 각 스티커는 서로 겹치지 않게 적당한 간격을 두고 배치해 주세요.\n각 스티커에는 흰색 칼선 느낌의 외곽 테두리를 분명하게 넣어 주세요.\n\n중요하게, 흰색 외곽선이 잘 보이도록 전체 시트의 바탕 배경은 흰색이 아닌 아주 연한 베이지색 단색 배경으로 표현해 주세요.\n배경은 복잡한 무늬 없이 밝고 부드러운 연베이지 톤이어야 하며, 스티커가 잘 돋보이도록 깔끔하게 처리해 주세요.\n예시 느낌: 아주 연한 크림베이지 / 부드러운 종이 느낌의 밝은 베이지.\n\n각 스티커는 모두 서로 다른 표정, 포즈, 문구, 장식, 여름 소품을 가져야 합니다.\n아래 요소들은 매번 임의로 조합을 바꿔 새로운 세트처럼 구성해 주세요.\n\n표정 후보:\n더위에 지침, 시원해서 행복함, 멍함, 졸림, 여유, 만족, 반짝이는 눈, 장난기, 설렘, 늘어짐\n\n포즈 후보:\n정면, 고개 갸웃, 에어컨 바람 맞기, 선풍기 앞에 앉기, 배를 보이고 눕기, 쿠션 위에 앉기, 수박 옆에 앉기, 얼음물 옆에 앉기, 두 앞발 모으기, 낮잠 자기\n\n문구 후보:\n집이 최고!, 에어컨이 최고!, 더워서 녹아요, 시원해요~, 집콕 중이에요, 오늘은 안 나가요, 낮잠 타임!, 수박 먹어요!, 엄마도 쉬세요, 여기가 피서지!, 뒹굴뒹굴, 너무 좋아요!, 행복한 휴가, 시원한 집이 최고!, 침대에서 놀아요!, 귀찮아요\n\n장식/소품 후보:\n에어컨 바람 표시, 선풍기, 수박, 얼음물, 부채, 쿠션, 냉감방석 느낌 소품, 아이스크림, 반짝이, 하트, 작은 담요, 리모컨, 냉방 아이콘, 물방울, 체크 쿠션\n\n전체 분위기는 귀엽고 밝고 선명한 상업용 스티커 느낌으로 표현해 주세요.\n각 스티커는 감정 표현이 잘 드러나고, 카카오톡 이모티콘처럼 직관적이고 사랑스럽게 만들어 주세요.\n문구는 각 스티커 아래 또는 어울리는 위치에 짧고 잘 읽히는 한국어 텍스트로 넣어 주세요.\n텍스트 색상은 스티커마다 어울리게 다양하게 적용하되, 전체적으로 통일감 있게 구성해 주세요.\n\n주의사항:\n같은 반려동물로 반드시 통일감 있게 표현할 것\n16개 모두 서로 다른 느낌으로 만들 것\n흰 칼선 외곽이 배경과 명확히 구분되도록 할 것\n실사 배경, 손, 책상, SNS UI, 그림자 효과 없이 오직 스티커 시트만 표현할 것\n완성 이미지는 스티커 상품 제작용 시안처럼 깔끔하고 선명하게 표현할 것\n\n[필수: AR 4:5]"
      },
      {
        "src": "images/p039-03.png",
        "title": "여름엔 워터파크",
        "caption": "이미지마다 프롬프트가 달라요",
        "prompt": "첨부한 반려동물 사진과 반드시 같은 반려동물처럼 보이는 여름 워터파크 테마의 귀여운 한국식 이모티콘 스티커 시트를 만들어 주세요.\n반려동물의 털색, 줄무늬, 얼굴형, 눈, 코, 귀, 볼살, 체형, 목걸이를 최대한 그대로 유지해 주세요.\n16개의 스티커는 모두 같은 반려동물로 보이되, 복붙처럼 똑같아 보이지 않게 표정, 포즈, 문구, 소품을 다양하게 바꿔 주세요.\n\n전체 이미지는 상업용 스티커 시트 디자인처럼 깔끔하고 완성도 높게 표현해 주세요.\n16개의 개별 스티커를 4x4 배열로 배치하고, 각 스티커는 서로 겹치지 않게 적당한 간격을 두고 배치해 주세요.\n각 스티커에는 흰색 칼선 느낌의 외곽 테두리를 분명하게 넣어 주세요.\n\n중요하게, 흰색 외곽선이 잘 보이도록 전체 시트의 바탕 배경은 흰색이 아닌 아주 연한 베이지색 단색 배경으로 표현해 주세요.\n배경은 복잡한 무늬 없이 밝고 부드러운 연베이지 톤이어야 하며, 스티커가 잘 돋보이도록 깔끔하게 처리해 주세요.\n예시 느낌: 아주 연한 크림베이지 / 부드러운 종이 느낌의 밝은 베이지.\n\n각 스티커는 모두 서로 다른 표정, 포즈, 문구, 장식, 여름 소품을 가져야 합니다.\n아래 요소들은 매번 임의로 조합을 바꿔 새로운 세트처럼 구성해 주세요.\n\n표정 후보:\n신남, 설렘, 놀람, 더위에 지침, 시원해서 행복함, 반짝이는 눈, 장난기, 멍함, 여유, 졸림\n\n포즈 후보:\n정면, 고개 갸웃, 튜브 위에 앉기, 물장구 치기, 슬라이드 타기 직전 포즈, 선글라스 쓰기, 파라솔 아래 쉬기, 비치볼 옆에 앉기, 두 앞발 모으기, 낮잠 자기\n\n문구 후보:\n물놀이 갈까요?, 워터파크예요!, 튜브는 필수!, 너무 시원해요~, 오늘은 휴가!, 슬라이드 타요!, 더워서 녹아요, 엄마도 쉬세요, 행복한 휴가, 사진 찍어 주세요!, 놀 준비 끝!, 첨벙첨벙!, 수영장이 최고!, 시원해서 좋아요, 신나요!, 바캉스예요!\n\n장식/소품 후보:\n튜브, 물방울, 비치볼, 파라솔, 선글라스, 아이스크림, 수박, 부채, 반짝이, 하트, 파도, 슬라이드, 야자수, 구명조끼 느낌 소품, 물총, 시원한 음료\n\n전체 분위기는 귀엽고 밝고 선명한 상업용 스티커 느낌으로 표현해 주세요.\n각 스티커는 감정 표현이 잘 드러나고, 카카오톡 이모티콘처럼 직관적이고 사랑스럽게 만들어 주세요.\n문구는 각 스티커 아래 또는 어울리는 위치에 짧고 잘 읽히는 한국어 텍스트로 넣어 주세요.\n텍스트 색상은 스티커마다 어울리게 다양하게 적용하되, 전체적으로 통일감 있게 구성해 주세요.\n\n주의사항:\n같은 반려동물로 반드시 통일감 있게 표현할 것\n16개 모두 서로 다른 느낌으로 만들 것\n흰 칼선 외곽이 배경과 명확히 구분되도록 할 것\n실사 배경, 손, 책상, SNS UI, 그림자 효과 없이 오직 스티커 시트만 표현할 것\n완성 이미지는 스티커 상품 제작용 시안처럼 깔끔하고 선명하게 표현할 것\n\n[필수: AR 4:5]"
      },
      {
        "src": "images/p039-04.png",
        "title": "여름엔 바다",
        "caption": "이미지마다 프롬프트가 달라요",
        "prompt": "첨부한 반려동물 사진과 반드시 같은 반려동물처럼 보이는 여름휴가 테마의 귀여운 한국식 이모티콘 스티커 시트를 만들어 주세요.\n반려동물의 털색, 줄무늬, 얼굴형, 눈, 코, 귀, 볼살, 체형, 목걸이를 최대한 그대로 유지해 주세요.\n16개의 스티커는 모두 같은 반려동물로 보이되, 복붙처럼 똑같아 보이지 않게 표정, 포즈, 문구, 소품을 다양하게 바꿔 주세요.\n\n전체 이미지는 상업용 스티커 시트 디자인처럼 깔끔하고 완성도 높게 표현해 주세요.\n16개의 개별 스티커를 4x4 배열로 배치하고, 각 스티커는 서로 겹치지 않게 적당한 간격을 두고 배치해 주세요.\n각 스티커에는 흰색 칼선 느낌의 외곽 테두리를 분명하게 넣어 주세요.\n\n중요하게, 흰색 외곽선이 잘 보이도록 전체 시트의 바탕 배경은 흰색이 아닌 아주 연한 베이지색 단색 배경으로 표현해 주세요.\n배경은 복잡한 무늬 없이 밝고 부드러운 연베이지 톤이어야 하며, 스티커가 잘 돋보이도록 깔끔하게 처리해 주세요.\n예시 느낌: 아주 연한 크림베이지 / 부드러운 종이 느낌의 밝은 베이지.\n\n각 스티커는 모두 서로 다른 표정, 포즈, 문구, 장식, 여름 소품을 가져야 합니다.\n아래 요소들은 매번 임의로 조합을 바꿔 새로운 세트처럼 구성해 주세요.\n\n표정 후보:\n신남, 설렘, 더위에 지침, 시원해서 행복함, 졸림, 놀람, 반짝이는 눈, 장난기, 멍함, 여유\n\n포즈 후보:\n정면, 고개 갸웃, 앞발 들기, 두 앞발 모으기, 튜브 위에 앉기, 물장구 치기, 파라솔 아래 쉬기, 선글라스 쓰기, 수박 옆에 앉기, 낮잠 자기\n\n문구 후보:\n여름이에요!, 휴가 출발!, 바다 가요!, 너무 시원해요~, 수박 먹어요!, 튜브는 필수!, 오늘은 휴가!, 더워서 녹아요, 엄마도 쉬세요, 물놀이 갈까요?, 행복한 휴가, 낮잠 타임!, 떠나요!, 여행 중이에요, 놀 준비 끝!, 사진 찍어 주세요!\n\n장식/소품 후보:\n햇빛, 파도, 물방울, 조개, 불가사리, 야자수, 수박, 아이스크림, 비치볼, 선글라스, 부채, 여행 가방, 파라솔, 반짝이, 하트\n\n전체 분위기는 귀엽고 밝고 선명한 상업용 스티커 느낌으로 표현해 주세요.\n각 스티커는 감정 표현이 잘 드러나고, 카카오톡 이모티콘처럼 직관적이고 사랑스럽게 만들어 주세요.\n문구는 각 스티커 아래 또는 어울리는 위치에 짧고 잘 읽히는 한국어 텍스트로 넣어 주세요.\n텍스트 색상은 스티커마다 어울리게 다양하게 적용하되, 전체적으로 통일감 있게 구성해 주세요.\n\n주의사항:\n\n같은 반려동물로 반드시 통일감 있게 표현할 것\n16개 모두 서로 다른 느낌으로 만들 것\n흰 칼선 외곽이 배경과 명확히 구분되도록 할 것\n실사 배경, 손, 책상, SNS UI, 그림자 효과 없이 오직 스티커 시트만 표현할 것\n완성 이미지는 스티커 상품 제작용 시안처럼 깔끔하고 선명하게 표현할 것\n\n[필수: AR 4:5]"
      },
      {
        "src": "images/p039-05.png",
        "title": "여름엔 호캉스",
        "caption": "이미지마다 프롬프트가 달라요",
        "prompt": "첨부한 반려동물 사진과 반드시 같은 반려동물처럼 보이는 여름 호캉스 테마의 귀여운 한국식 이모티콘 스티커 시트를 만들어 주세요.\n반려동물의 털색, 줄무늬, 얼굴형, 눈, 코, 귀, 볼살, 체형, 목걸이를 최대한 그대로 유지해 주세요.\n16개의 스티커는 모두 같은 반려동물로 보이되, 복붙처럼 똑같아 보이지 않게 표정, 포즈, 문구, 소품을 다양하게 바꿔 주세요.\n\n전체 이미지는 상업용 스티커 시트 디자인처럼 깔끔하고 완성도 높게 표현해 주세요.\n16개의 개별 스티커를 4x4 배열로 배치하고, 각 스티커는 서로 겹치지 않게 적당한 간격을 두고 배치해 주세요.\n각 스티커에는 흰색 칼선 느낌의 외곽 테두리를 분명하게 넣어 주세요.\n\n중요하게, 흰색 외곽선이 잘 보이도록 전체 시트의 바탕 배경은 흰색이 아닌 아주 연한 베이지색 단색 배경으로 표현해 주세요.\n배경은 복잡한 무늬 없이 밝고 부드러운 연베이지 톤이어야 하며, 스티커가 잘 돋보이도록 깔끔하게 처리해 주세요.\n예시 느낌: 아주 연한 크림베이지 / 부드러운 종이 느낌의 밝은 베이지.\n\n각 스티커는 모두 서로 다른 표정, 포즈, 문구, 장식, 여름 소품을 가져야 합니다.\n아래 요소들은 매번 임의로 조합을 바꿔 새로운 세트처럼 구성해 주세요.\n\n표정 후보:\n신남, 설렘, 여유, 졸림, 만족, 반짝이는 눈, 장난기, 멍함, 시원해서 행복함, 뿌듯함\n\n포즈 후보:\n정면, 고개 갸웃, 침대에 누워 있기, 베개 끌어안기, 창가에 앉기, 룸서비스 기다리기, 수건 두르고 있기, 선글라스 쓰기, 수영장 튜브 위에 앉기, 낮잠 자기\n\n문구 후보:\n호캉스예요!, 오늘은 쉬는 날!, 침대가 최고!, 룸서비스 주세요!, 시원해요~, 수영장 가요!, 엄마도 쉬세요, 낮잠 타임!, 행복한 휴가, 체크인 완료!, 뒹굴뒹굴, 사진 찍어 주세요!, 여행 중이에요, 너무 좋아요!, 놀 준비 끝!, 여유 만끽!\n\n장식/소품 후보:\n베개, 침대, 수건, 욕실 가운, 선글라스, 여행 가방, 키카드, 미니 수영장 튜브, 트레이, 음료, 아이스크림, 파라솔, 반짝이, 하트, 창문, 야자수 화분\n\n전체 분위기는 귀엽고 밝고 선명한 상업용 스티커 느낌으로 표현해 주세요.\n각 스티커는 감정 표현이 잘 드러나고, 카카오톡 이모티콘처럼 직관적이고 사랑스럽게 만들어 주세요.\n문구는 각 스티커 아래 또는 어울리는 위치에 짧고 잘 읽히는 한국어 텍스트로 넣어 주세요.\n텍스트 색상은 스티커마다 어울리게 다양하게 적용하되, 전체적으로 통일감 있게 구성해 주세요.\n\n주의사항:\n같은 반려동물로 반드시 통일감 있게 표현할 것\n16개 모두 서로 다른 느낌으로 만들 것\n흰 칼선 외곽이 배경과 명확히 구분되도록 할 것\n실사 배경, 손, 책상, SNS UI, 그림자 효과 없이 오직 스티커 시트만 표현할 것\n완성 이미지는 스티커 상품 제작용 시안처럼 깔끔하고 선명하게 표현할 것\n\n[필수: AR 4:5]"
      },
      {
        "src": "images/p039-06.png",
        "title": "여름엔 계곡",
        "caption": "이미지마다 프롬프트가 달라요",
        "prompt": "첨부한 반려동물 사진과 반드시 같은 반려동물처럼 보이는 여름 계곡 피서 테마의 귀여운 한국식 이모티콘 스티커 시트를 만들어 주세요.\n반려동물의 털색, 줄무늬, 얼굴형, 눈, 코, 귀, 볼살, 체형, 목걸이를 최대한 그대로 유지해 주세요.\n16개의 스티커는 모두 같은 반려동물로 보이되, 복붙처럼 똑같아 보이지 않게 표정, 포즈, 문구, 소품을 다양하게 바꿔 주세요.\n\n전체 이미지는 상업용 스티커 시트 디자인처럼 깔끔하고 완성도 높게 표현해 주세요.\n16개의 개별 스티커를 4x4 배열로 배치하고, 각 스티커는 서로 겹치지 않게 적당한 간격을 두고 배치해 주세요.\n각 스티커에는 흰색 칼선 느낌의 외곽 테두리를 분명하게 넣어 주세요.\n\n중요하게, 흰색 외곽선이 잘 보이도록 전체 시트의 바탕 배경은 흰색이 아닌 아주 연한 베이지색 단색 배경으로 표현해 주세요.\n배경은 복잡한 무늬 없이 밝고 부드러운 연베이지 톤이어야 하며, 스티커가 잘 돋보이도록 깔끔하게 처리해 주세요.\n예시 느낌: 아주 연한 크림베이지 / 부드러운 종이 느낌의 밝은 베이지.\n\n각 스티커는 모두 서로 다른 표정, 포즈, 문구, 장식, 여름 소품을 가져야 합니다.\n아래 요소들은 매번 임의로 조합을 바꿔 새로운 세트처럼 구성해 주세요.\n\n표정 후보:\n신남, 설렘, 시원해서 행복함, 더위에 지침, 졸림, 놀람, 반짝이는 눈, 장난기, 멍함, 여유\n\n포즈 후보:\n정면, 고개 갸웃, 앞발 들기, 두 앞발 모으기, 계곡물에 발 담그기, 물장구 치기, 바위 위에 앉기, 그늘 아래 쉬기, 수박 옆에 앉기, 낮잠 자기\n\n문구 후보:\n계곡 왔어요!, 물이 시원해요~, 발 담그는 중이에요, 여기가 천국!, 수박 먹어요!, 오늘은 피서!, 더위 탈출!, 물놀이 최고!, 엄마도 쉬세요, 바위 명당이에요, 낮잠 타임!, 풍경이 좋아요, 놀 준비 끝!, 사진 찍어 주세요!, 행복한 피서, 물소리가 좋아요\n\n장식/소품 후보:\n계곡물, 물방울, 바위, 나뭇잎, 수박, 부채, 반짝이, 하트, 조약돌, 작은 폭포, 그늘, 대나무 바구니, 튜브, 아이스 음료, 발수건\n\n전체 분위기는 귀엽고 밝고 선명한 상업용 스티커 느낌으로 표현해 주세요.\n각 스티커는 감정 표현이 잘 드러나고, 카카오톡 이모티콘처럼 직관적이고 사랑스럽게 만들어 주세요.\n문구는 각 스티커 아래 또는 어울리는 위치에 짧고 잘 읽히는 한국어 텍스트로 넣어 주세요.\n텍스트 색상은 스티커마다 어울리게 다양하게 적용하되, 전체적으로 통일감 있게 구성해 주세요.\n\n주의사항:\n같은 반려동물로 반드시 통일감 있게 표현할 것\n16개 모두 서로 다른 느낌으로 만들 것\n흰 칼선 외곽이 배경과 명확히 구분되도록 할 것\n실사 배경, 손, 책상, SNS UI, 그림자 효과 없이 오직 스티커 시트만 표현할 것\n완성 이미지는 스티커 상품 제작용 시안처럼 깔끔하고 선명하게 표현할 것\n\n[필수: AR 4:5]"
      }
    ]
  },
  {
    "id": "P-038",
    "animal": "small",
    "title": "휴가 스탬프 햄님",
    "category": "사진",
    "cover": "images/p038-01.png",
    "prompt": "첨부한 반려동물 사진을 주인공으로 사용해 주세요. 사진 속 반려동물의 종류, 털색, 무늬, 얼굴형, 눈 색과 모양, 코와 입, 귀 모양, 볼살, 체형을 최대한 그대로 유지해 주세요. 반드시 실제 사진 속 반려동물과 같은 반려동물처럼 보여야 합니다.\n\n입력한 여행지를 기념하는 원형 여행 스탬프 / 기념 도장 디자인을 만들어 주세요. 전체 이미지는 마치 여행지에서 수집하는 기념 스탬프, 여행 인증 도장, 빈티지 트래블 배지처럼 보여야 합니다.\n\n도장의 중앙 주인공은 반려동물이어야 하며, 단순히 배경 앞에 서 있는 것이 아니라 여행 기념 도장 속 마스코트 캐릭터처럼 표현해 주세요. 반려동물은 정면을 바라보는 귀엽고 사랑스러운 모습으로, 얼굴이 한눈에 잘 보이고 존재감 있게 배치해 주세요. 표정은 또렷하고 호감 가는 자연스러운 표정으로 해 주세요.\n\n도장 안에는 입력한 여행지를 떠올릴 수 있는 대표적인 상징 요소를 2~4개 정도 넣어 주세요. 예를 들어 랜드마크, 자연 풍경, 음식, 지역 상징물 등을 간결하고 예쁘게 배치해 주세요. 요소가 너무 많아 복잡해지지 않도록 하고, 한눈에 여행지 분위기가 느껴지게 해 주세요.\n\n도장 윗부분 또는 테두리에는 여행지명이 크고 선명하게 들어가야 합니다. 아래쪽에는 날짜가 실제 여행 기록처럼 보이도록 깔끔하고 정확하게 표시되어야 합니다.\n\n만약 아래 입력란에 여행지명이나 날짜가 비어 있다면, 현재 대화에서 가장 최근에 언급된 여행지명과 날짜를 우선 참고해 자연스럽게 반영해 주세요. 현재 대화 안에서도 명확하게 찾을 수 없다면, 여행지명은 여행 기념 스탬프에 어울리는 자연스러운 장소명으로 표현하고, 날짜는 스탬프에 어울리는 깔끔한 날짜 형식으로 자연스럽게 넣어 주세요. 어떤 경우에도 여행지명과 날짜가 비어 보이지 않게 완성해 주세요.\n\n전체 도장 형태는 원형이며, 테두리가 분명하고 실제 잉크 도장처럼 약간 빈티지한 질감이 느껴지면 좋습니다. 하지만 전체 인상은 지저분하지 않고 귀엽고 완성도 높은 굿즈 느낌이어야 합니다.\n\n색감은 여행지 분위기에 어울리게 정하고, 전체적으로 수집하고 싶은 기념 스탬프 / 굿즈 / 스티커 같은 느낌으로 표현해 주세요.\n\n배경은 밝은 크림색 또는 종이 질감의 심플한 배경으로 하고, 주변에는 아주 옅게 여권 스탬프, 우편 소인, 물결선, 작은 아이콘 등 여행 기록 느낌의 장식 요소를 넣어도 좋습니다. 하지만 메인 도장이 가장 눈에 띄어야 합니다.\n\n전체 이미지는 세로 4:5 비율, 여백과 SNS UI 없이 깔끔하게 표현해 주세요.\n\n여행지명: [여기에 입력]\n날짜: [여기에 입력]",
    "images": [
      {
        "src": "images/p038-01.png"
      }
    ]
  },
  {
    "id": "P-037",
    "animal": "small",
    "title": "햄님이 미니 코스튬",
    "category": "사진",
    "cover": "images/p037-01.png",
    "prompt": "첨부한 반려동물 사진을 사용해 주세요. 사진 속 반려동물의 털색, 무늬, 얼굴형, 눈, 코, 입, 귀, 볼살, 체형을 최대한 그대로 유지해 주세요. 반드시 실제 사진과 같은 반려동물처럼 보여야 합니다.\n\n반려동물이 사람의 손가락 끝 위에 올라간 미니 사이즈로 표현된 귀엽고 사실적인 사진을 만들어 주세요. 반려동물은 손가락의 중간 부분이 아니라 반드시 손가락 맨 끝의 둥근 부분 위에 올라가 있어야 합니다. 너무 작게 점처럼 보이지 않도록 해 주세요. 손가락 끝 위에서 존재감 있게 보이되, 여전히 “손가락 위에 올라간 작은 반려동물”처럼 느껴져야 합니다. 반려동물 몸집은 손가락 끝 너비의 약 60~75% 정도로 보이게 해 주세요. 손가락 피부가 반려동물 주변으로 자연스럽게 충분히 보이도록 해 주세요.\n\nAdditional handling notes: determine a single costume variation by using the current local time at the moment this request is processed. Read the time as a 24-hour HHMM value, add the four digits together, and use the remainder when divided by 10 to choose exactly one costume. Do not explain the rule, do not mention the calculation, and do not output any text about the selection. Apply only one costume, never mix multiple costumes. Use this mapping: 0 bee costume, 1 bunny costume, 2 chick costume, 3 frog raincoat costume, 4 strawberry costume, 5 astronaut costume, 6 wizard costume, 7 angel costume, 8 prince or princess costume, 9 shark hood costume. Also make the facial mood and atmosphere naturally match the selected costume.\n\n반려동물은 손가락 끝 위에 안정감 있게 앉아 있거나 앞발을 모으고 살짝 기대 앉은 자세로 표현해 주세요. 반려동물에게는 귀엽고 작은 코스튬 또는 테마 의상 1종만 적용해 주세요. 의상은 봉제 인형 코스튬처럼 사랑스럽고 아기자기하게 표현해 주세요. 반려동물 얼굴은 최대한 선명하고 매력적으로 보여야 하며, 손가락과 반려동물의 크기 대비가 한눈에 느껴져야 합니다.\n\n배경은 깔끔하고 자연스럽게 흐려진 밝은 실내 또는 중성 배경으로 해 주세요. 전체 이미지는 스마트폰으로 찍은 듯한 사실적인 느낌으로 표현해 주세요. 세로 4:5 비율, 여백 없이, 글자, 말풍선, 워터마크, SNS 화면 요소 없이 표현해 주세요.",
    "images": [
      {
        "src": "images/p037-01.png"
      },
      {
        "src": "images/p037-02.png"
      },
      {
        "src": "images/p037-03.png"
      },
      {
        "src": "images/p037-04.png"
      },
      {
        "src": "images/p037-05.png"
      },
      {
        "src": "images/p037-06.png"
      },
      {
        "src": "images/p037-07.png"
      },
      {
        "src": "images/p037-08.png"
      },
      {
        "src": "images/p037-09.png"
      },
      {
        "src": "images/p037-10.png"
      }
    ]
  },
  {
    "id": "P-036",
    "animal": "small",
    "title": "햄님 블록",
    "category": "사진",
    "cover": "images/p036-01.png",
    "prompt": "첨부한 반려동물 사진을 사용해 주세요. 사진 속 반려동물의 털색, 무늬, 얼굴형, 눈 색, 코와 입 모양, 귀 모양, 볼살, 체형을 최대한 그대로 유지해 주세요. 반드시 실제 사진 속 반려동물과 같은 반려동물처럼 보여야 합니다.\n\n실제 반려동물을 본떠 만든 맞춤형 브릭 반려동물 세트 패키지 사진을 만들어 주세요. 바닥에는 무광 검은색의 고급 조립 블록 제품 상자가 세로로 세워져 있고, 상자 윗부분만 열려 있습니다. 상자 안에는 실제 반려동물이 들어가 있으며, 머리와 턱 아래, 가슴 윗부분만 상자 위로 작게 빼꼼 보이게 해 주세요. 몸통과 등은 대부분 상자 안에 가려져 있어야 합니다. 표정은 시큰둥하거나 어리둥절한 자연스러운 반려동물 표정으로 해 주세요.\n\n상자 앞면 중앙에는 첨부한 반려동물과 동일한 반려동물을 작은 브릭 블록으로 정교하게 만든 완성품 이미지가 크게 인쇄되어 있어야 합니다. 단색 피규어가 아니라 실제 반려동물의 털색, 무늬, 얼굴 음영, 눈 색까지 여러 색 브릭으로 세밀하게 구현된 맞춤 피규어처럼 보여야 합니다. 하단에는 같은 브릭 반려동물의 정면, 45도, 옆모습 작은 이미지 3개를 넣어 주세요.\n\n상자 왼쪽 위에는 빨간 정사각형 안에 “TOY”라고 적힌 가상 로고를 넣어 주세요. 오른쪽 위와 측면에는 CUSTOM PET, BRICK PORTRAIT SERIES, 16+, 2480 PCS 같은 짧고 읽을 수 있는 영문 제품 정보를 자연스럽게 넣어 주세요. 상자 옆 바닥에는 반려동물 털색과 비슷한 작은 브릭 부품이 담긴 투명 비닐 봉투를 작게 놓아 주세요.\n\n카메라는 위에서 살짝 내려다보는 비스듬한 스마트폰 사진 구도로 해 주세요. 상자 앞면과 옆면, 열린 상자 날개가 함께 보여야 합니다. 배경은 밝은 중성톤 카펫이 깔린 집 안이며, 전체적으로 광고 화보보다 보호자가 우연히 찍은 귀엽고 현실적인 밈 사진 느낌으로 표현해 주세요.\n\n세로 4:5 비율, 사실적인 사진, 사람 금지, 반려동물 추가 생성 금지, 일반 택배 상자 느낌 금지, 워터마크·SNS 화면·실제 브랜드 로고 복제 금지.",
    "images": [
      {
        "src": "images/p036-01.png"
      }
    ]
  },
  {
    "id": "P-035",
    "animal": "small",
    "title": "꽃 햄님",
    "category": "사진",
    "cover": "images/p035-01.png",
    "prompt": "첨부한 반려동물 사진을 주인공으로 사용해 주세요. 사진 속 반려동물의 털색, 무늬, 얼굴형, 눈 색, 코와 입 모양, 귀 모양을 최대한 그대로 유지해 주세요. 반드시 사진 속 반려동물과 같은 반려동물처럼 보여야 합니다. 반려동물이 커다란 생화 꽃다발 한가운데에 폭 파묻혀 있는 귀엽고 사랑스러운 사진을 만들어 주세요. 꽃다발은 사람이 두 팔로 안아야 할 정도로 크고 풍성해야 하며, 반려동물의 머리와 목, 가슴 윗부분만 꽃 사이로 자연스럽게 드러나게 해 주세요. 반려동물이 꽃다발 앞에 따로 앉아 있거나 꽃 위에 올라탄 모습이 아니라, 실제로 꽃다발의 중심에 포근하게 들어가 얼굴만 빼꼼 내민 모습이어야 합니다. 반려동물 얼굴 주변을 크고 탐스러운 꽃들이 둥글게 감싸도록 배치해 주세요. 꽃잎이나 잎사귀가 눈과 코를 가리지 않게 하고, 얼굴은 이미지의 가장 중요한 중심으로 또렷하게 보여 주세요. 꽃의 색상은 한 가지 계열을 중심으로 통일해 주세요. 생성할 때마다 아래 색상 중 하나를 자연스럽게 선택해 주세요. 따뜻하고 화사한 노란색 꽃다발 부드럽고 사랑스러운 연분홍색 꽃다발 깨끗하고 청량한 하늘색과 흰색 꽃다발 우아한 크림색과 연보라색 꽃다발 꽃은 장미, 거베라, 카네이션, 작은 들꽃과 봉오리 등을 풍성하게 섞되, 지나치게 복잡하거나 빽빽하게 뭉개져 보이지 않도록 해 주세요. 각각의 꽃 형태가 자연스럽고 분명하게 보여야 합니다. 반려동물은 카메라를 정면으로 바라보며, 조금 놀란 듯 동그란 눈이나 얌전하고 웃는듯한 표정을 짓고 있습니다. 사람처럼 웃는 표정이나 과장된 인형 같은 표정은 피하고, 실제 반려동물 특유의 귀엽고 엉뚱한 분위기를 살려 주세요. 꽃다발은 고급스럽고 깔끔한 무광 포장지로 감싸고, 화면 아래쪽에는 꽃다발을 안고 있는 사람의 손이나 팔 일부만 자연스럽게 보이게 해 주세요. 사람의 얼굴과 몸 전체는 나오지 않게 해 주세요. 밝고 부드러운 자연광, 깨끗한 실내 배경, 따뜻하고 화사한 색감, 실제 스마트폰이나 카메라로 촬영한 듯한 선명한 반려동물 사진으로 표현해 주세요. 인스타그램 피드에 잘 어울리는 세로 4:5 비율로 만들어 주세요. 꽃다발과 반려동물이 화면을 풍성하게 채우도록 하고, 위아래 여백이나 테두리는 넣지 마세요. 글자, 로고, 워터마크, SNS 화면, 리본 문구는 넣지 마세요. 반려동물의 몸이 꽃으로 변하거나 꽃과 합쳐지지 않게 해 주세요. 꽃잎으로 된 귀, 꽃 모양 눈, 사람 손처럼 변한 앞발, 비정상적으로 많은 꽃잎, 찌그러진 꽃, 중복된 귀와 눈은 표현하지 마세요.",
    "images": [
      {
        "src": "images/p035-01.png"
      }
    ]
  },
  {
    "id": "P-034",
    "animal": "small",
    "title": "꽃 화보 햄님",
    "category": "사진",
    "cover": "images/p034-01.png",
    "prompt": "첨부한 반려동물 사진을 주인공으로 사용해 주세요.\n\n사진 속 반려동물의 털색, 무늬, 얼굴형, 눈 색, 귀 모양, 체형을 최대한 그대로 유지해 주세요.\n반드시 같은 반려동물처럼 보여야 합니다.\n\n밝고 화사한 야외 반려동물 화보 사진을 만들어 주세요.\n전체 분위기는 깨끗하고 맑고 따뜻한 봄~초여름 감성입니다.\n배경은 벽, 담장, 창문가, 조용한 골목 중 하나처럼 깔끔하고 단정한 공간으로 표현해 주세요.\n\n반려동물 주변에는 화사한 꽃이 풍성하게 피어 있어야 합니다.\n꽃은 연한 하늘색, 노란색, 분홍색 계열 중 하나로 표현하고,\n벽을 따라 흐르거나, 위에서 아래로 늘어지거나, 담장 주변을 감싸는 식으로 자연스럽게 배치해 주세요.\n\n반려동물의 포즈와 구도는 자연스러운 화보 느낌 안에서 조금씩 다르게 해 주세요.\n예를 들면:\n담장 위에 앞발을 올리고 앉아 있는 모습,\n벽에 기대어 편안히 누워 있는 모습,\n꽃 아래에서 눈을 감고 햇살을 즐기는 모습,\n창문 밖으로 고개를 내미는 모습,\n살짝 위를 올려다보는 모습 등으로 표현해 주세요.\n\n카메라 구도도 고정하지 말고, 클로즈업, 반신샷, 전신샷 중 자연스럽게 선택해 주세요.\n하지만 전체적으로는 깨끗하고 미니멀한 감성 화보 톤을 유지해 주세요.\n\n빛은 부드러운 자연광, 선명하지만 부드러운 그림자, 맑은 하늘, 은은한 아웃포커싱이 느껴지게 해 주세요.\n복잡한 소품은 넣지 말고, 감성적인 반려동물 화보처럼 자연스럽고 아름답게 표현해 주세요.\n\n중요한 점은 반려동물의 정체성과 전체 화보 분위기는 유지하되,\n꽃 색, 꽃 배치, 구도, 포즈는 생성할 때마다 조금씩 다르게 나오는 것입니다.\n\n세로형 이미지, 인스타그램 감성 화보 느낌으로 만들어 주세요. 필수 : AR 4:5",
    "images": [
      {
        "src": "images/p034-01.png"
      }
    ]
  },
  {
    "id": "P-033",
    "animal": "small",
    "title": "계곡 햄님",
    "category": "사진",
    "cover": "images/p033-01.png",
    "prompt": "첨부한 반려동물 사진을 주인공으로 사용해 주세요. 사진 속 반려동물의 털색, 무늬, 얼굴형, 눈 색, 귀 모양과 체형을 그대로 유지해 주세요. 한여름의 맑고 시원한 계곡입니다. 반려동물이 아주 얕고 맑은 계곡물 위에 발라당 누워서 눈을 감고 편안하게 피서를 즐기고 있습니다. 반려동물은 몸의 일부가 얕은 물에 닿아 있고, 네 발은 힘을 뺀 채 자연스럽게 벌어져 있습니다. 표정은 매우 편안하고 나른하며, 눈을 감은 채 더위를 식히는 모습입니다. 배와 앞발, 뒷발이 귀엽게 드러나고, 온몸에 긴장이 풀린 듯한 자세로 누워 있습니다. 반려동물 주변의 물결은 아주 잔잔하게 퍼지고 있으며, 몸 주변에는 작은 물반사와 은은한 물결 표현이 있습니다. 투명한 계곡물 아래로 둥근 조약돌이 보이고, 주변에는 초록색 나뭇잎과 큰 바위가 놓여 있습니다. 여름 햇빛이 계곡물 위에서 반짝이고, 전체적으로 밝고 청량하며 평화로운 분위기로 표현해 주세요. 반려동물이 더 작고 귀엽게 보이도록 주변의 바위, 조약돌, 나뭇잎은 조금 크게 표현해 주세요. 전체 분위기는 유쾌하고 사랑스러운 여름 계곡 풍경입니다. 자연스럽고 선명한 이미지, 맑고 투명한 물, 귀엽고 편안한 반려동물의 모습이 잘 드러나게 해 주세요. 이미지 비율 4:5.",
    "images": [
      {
        "src": "images/p033-01.png"
      }
    ]
  },
  {
    "id": "P-032",
    "animal": "dog",
    "title": "여름 휴가 스탬프",
    "category": "굿즈",
    "cover": "images/p032-01.png",
    "description": "프롬프트 마지막에 다녀온 여행지와 날짜를 입력해주세요",
    "prompt": "첨부한 반려동물 사진을 주인공으로 사용해 주세요. 사진 속 반려동물의 종류, 털색, 무늬, 얼굴형, 눈 색과 모양, 코와 입, 귀 모양, 볼살, 체형을 최대한 그대로 유지해 주세요. 반드시 실제 사진 속 반려동물과 같은 반려동물처럼 보여야 합니다.\n\n입력한 여행지를 기념하는 원형 여행 스탬프 / 기념 도장 디자인을 만들어 주세요. 전체 이미지는 마치 여행지에서 수집하는 기념 스탬프, 여행 인증 도장, 빈티지 트래블 배지처럼 보여야 합니다.\n\n도장의 중앙 주인공은 반려동물이어야 하며, 단순히 배경 앞에 서 있는 것이 아니라 여행 기념 도장 속 마스코트 캐릭터처럼 표현해 주세요. 반려동물은 정면을 바라보는 귀엽고 사랑스러운 모습으로, 얼굴이 한눈에 잘 보이고 존재감 있게 배치해 주세요. 표정은 또렷하고 호감 가는 자연스러운 표정으로 해 주세요.\n\n도장 안에는 입력한 여행지를 떠올릴 수 있는 대표적인 상징 요소를 2~4개 정도 넣어 주세요. 예를 들어 랜드마크, 자연 풍경, 음식, 지역 상징물 등을 간결하고 예쁘게 배치해 주세요. 요소가 너무 많아 복잡해지지 않도록 하고, 한눈에 여행지 분위기가 느껴지게 해 주세요.\n\n도장 윗부분 또는 테두리에는 여행지명이 크고 선명하게 들어가야 합니다. 아래쪽에는 날짜가 실제 여행 기록처럼 보이도록 깔끔하고 정확하게 표시되어야 합니다.\n\n만약 아래 입력란에 여행지명이나 날짜가 비어 있다면, 현재 대화에서 가장 최근에 언급된 여행지명과 날짜를 우선 참고해 자연스럽게 반영해 주세요. 현재 대화 안에서도 명확하게 찾을 수 없다면, 여행지명은 여행 기념 스탬프에 어울리는 자연스러운 장소명으로 표현하고, 날짜는 스탬프에 어울리는 깔끔한 날짜 형식으로 자연스럽게 넣어 주세요. 어떤 경우에도 여행지명과 날짜가 비어 보이지 않게 완성해 주세요.\n\n전체 도장 형태는 원형이며, 테두리가 분명하고 실제 잉크 도장처럼 약간 빈티지한 질감이 느껴지면 좋습니다. 하지만 전체 인상은 지저분하지 않고 귀엽고 완성도 높은 굿즈 느낌이어야 합니다.\n\n색감은 여행지 분위기에 어울리게 정하고, 전체적으로 수집하고 싶은 기념 스탬프 / 굿즈 / 스티커 같은 느낌으로 표현해 주세요.\n\n배경은 밝은 크림색 또는 종이 질감의 심플한 배경으로 하고, 주변에는 아주 옅게 여권 스탬프, 우편 소인, 물결선, 작은 아이콘 등 여행 기록 느낌의 장식 요소를 넣어도 좋습니다. 하지만 메인 도장이 가장 눈에 띄어야 합니다.\n\n전체 이미지는 세로 4:5 비율, 여백과 SNS UI 없이 깔끔하게 표현해 주세요.\n\n여행지명: [여기에 입력]\n날짜: [여기에 입력]",
    "images": [
      {
        "src": "images/p032-01.png"
      }
    ]
  },
  {
    "id": "P-031",
    "animal": "cat",
    "title": "다녀온 휴가지 스탬프",
    "category": "굿즈",
    "cover": "images/p031-01.png",
    "description": "우리 고양이 사진을 넣고 다녀온 여행지와 방문 일자를 입력해주세요",
    "prompt": "첨부한 반려동물 사진을 주인공으로 사용해 주세요. 사진 속 반려동물의 종류, 털색, 무늬, 얼굴형, 눈 색과 모양, 코와 입, 귀 모양, 볼살, 체형을 최대한 그대로 유지해 주세요. 반드시 실제 사진 속 반려동물과 같은 반려동물처럼 보여야 합니다.\n\n입력한 여행지를 기념하는 원형 여행 스탬프 / 기념 도장 디자인을 만들어 주세요. 전체 이미지는 마치 여행지에서 수집하는 기념 스탬프, 여행 인증 도장, 빈티지 트래블 배지처럼 보여야 합니다.\n\n도장의 중앙 주인공은 반려동물이어야 하며, 단순히 배경 앞에 서 있는 것이 아니라 여행 기념 도장 속 마스코트 캐릭터처럼 표현해 주세요. 반려동물은 정면을 바라보는 귀엽고 사랑스러운 모습으로, 얼굴이 한눈에 잘 보이고 존재감 있게 배치해 주세요. 표정은 또렷하고 호감 가는 자연스러운 표정으로 해 주세요.\n\n도장 안에는 입력한 여행지를 떠올릴 수 있는 대표적인 상징 요소를 2~4개 정도 넣어 주세요. 예를 들어 랜드마크, 자연 풍경, 음식, 지역 상징물 등을 간결하고 예쁘게 배치해 주세요. 요소가 너무 많아 복잡해지지 않도록 하고, 한눈에 여행지 분위기가 느껴지게 해 주세요.\n\n도장 윗부분 또는 테두리에는 여행지명이 크고 선명하게 들어가야 합니다. 아래쪽에는 날짜가 실제 여행 기록처럼 보이도록 깔끔하고 정확하게 표시되어야 합니다.\n\n만약 아래 입력란에 여행지명이나 날짜가 비어 있다면, 현재 대화에서 가장 최근에 언급된 여행지명과 날짜를 우선 참고해 자연스럽게 반영해 주세요. 현재 대화 안에서도 명확하게 찾을 수 없다면, 여행지명은 여행 기념 스탬프에 어울리는 자연스러운 장소명으로 표현하고, 날짜는 스탬프에 어울리는 깔끔한 날짜 형식으로 자연스럽게 넣어 주세요. 어떤 경우에도 여행지명과 날짜가 비어 보이지 않게 완성해 주세요.\n\n전체 도장 형태는 원형이며, 테두리가 분명하고 실제 잉크 도장처럼 약간 빈티지한 질감이 느껴지면 좋습니다. 하지만 전체 인상은 지저분하지 않고 귀엽고 완성도 높은 굿즈 느낌이어야 합니다.\n\n색감은 여행지 분위기에 어울리게 정하고, 전체적으로 수집하고 싶은 기념 스탬프 / 굿즈 / 스티커 같은 느낌으로 표현해 주세요.\n\n배경은 밝은 크림색 또는 종이 질감의 심플한 배경으로 하고, 주변에는 아주 옅게 여권 스탬프, 우편 소인, 물결선, 작은 아이콘 등 여행 기록 느낌의 장식 요소를 넣어도 좋습니다. 하지만 메인 도장이 가장 눈에 띄어야 합니다.\n\n전체 이미지는 세로 4:5 비율, 여백과 SNS UI 없이 깔끔하게 표현해 주세요.\n\n여행지명: [여기에 입력]\n날짜: [여기에 입력]",
    "images": [
      {
        "src": "images/p031-01.png"
      }
    ]
  },
  {
    "id": "P-030",
    "animal": "dog",
    "title": "손가락 끝 요정",
    "category": "이미지",
    "cover": "images/p030-01.png",
    "description": "만들때마다 다른 열가지 코스튬",
    "prompt": "첨부한 반려동물 사진을 사용해 주세요. 사진 속 반려동물의 털색, 무늬, 얼굴형, 눈, 코, 입, 귀, 볼살, 체형을 최대한 그대로 유지해 주세요. 반드시 실제 사진과 같은 반려동물처럼 보여야 합니다.\n\n반려동물이 사람의 손가락 끝 위에 올라간 미니 사이즈로 표현된 귀엽고 사실적인 사진을 만들어 주세요. 반려동물은 손가락의 중간 부분이 아니라 반드시 손가락 맨 끝의 둥근 부분 위에 올라가 있어야 합니다. 너무 작게 점처럼 보이지 않도록 해 주세요. 손가락 끝 위에서 존재감 있게 보이되, 여전히 “손가락 위에 올라간 작은 반려동물”처럼 느껴져야 합니다. 반려동물 몸집은 손가락 끝 너비의 약 60~75% 정도로 보이게 해 주세요. 손가락 피부가 반려동물 주변으로 자연스럽게 충분히 보이도록 해 주세요.\n\nAdditional handling notes: determine a single costume variation by using the current local time at the moment this request is processed. Read the time as a 24-hour HHMM value, add the four digits together, and use the remainder when divided by 10 to choose exactly one costume. Do not explain the rule, do not mention the calculation, and do not output any text about the selection. Apply only one costume, never mix multiple costumes. Use this mapping: 0 bee costume, 1 bunny costume, 2 chick costume, 3 frog raincoat costume, 4 strawberry costume, 5 astronaut costume, 6 wizard costume, 7 angel costume, 8 prince or princess costume, 9 shark hood costume. Also make the facial mood and atmosphere naturally match the selected costume.\n\n반려동물은 손가락 끝 위에 안정감 있게 앉아 있거나 앞발을 모으고 살짝 기대 앉은 자세로 표현해 주세요. 반려동물에게는 귀엽고 작은 코스튬 또는 테마 의상 1종만 적용해 주세요. 의상은 봉제 인형 코스튬처럼 사랑스럽고 아기자기하게 표현해 주세요. 반려동물 얼굴은 최대한 선명하고 매력적으로 보여야 하며, 손가락과 반려동물의 크기 대비가 한눈에 느껴져야 합니다.\n\n배경은 깔끔하고 자연스럽게 흐려진 밝은 실내 또는 중성 배경으로 해 주세요. 전체 이미지는 스마트폰으로 찍은 듯한 사실적인 느낌으로 표현해 주세요. 세로 4:5 비율, 여백 없이, 글자, 말풍선, 워터마크, SNS 화면 요소 없이 표현해 주세요.",
    "images": [
      {
        "src": "images/p030-01.png"
      },
      {
        "src": "images/p030-02.png"
      },
      {
        "src": "images/p030-03.png"
      },
      {
        "src": "images/p030-04.png"
      },
      {
        "src": "images/p030-05.png"
      },
      {
        "src": "images/p030-06.png"
      },
      {
        "src": "images/p030-07.png"
      },
      {
        "src": "images/p030-08.png"
      },
      {
        "src": "images/p030-09.png"
      }
    ]
  },
  {
    "id": "P-029",
    "animal": "dog",
    "title": "강아지 블록",
    "category": "이미지",
    "cover": "images/p029-01.png",
    "description": "강아지 블록",
    "prompt": "첨부한 반려동물 사진을 사용해 주세요. 사진 속 반려동물의 털색, 무늬, 얼굴형, 눈 색, 코와 입 모양, 귀 모양, 볼살, 체형을 최대한 그대로 유지해 주세요. 반드시 실제 사진 속 반려동물과 같은 반려동물처럼 보여야 합니다.\n\n실제 반려동물을 본떠 만든 맞춤형 브릭 반려동물 세트 패키지 사진을 만들어 주세요. 바닥에는 무광 검은색의 고급 조립 블록 제품 상자가 세로로 세워져 있고, 상자 윗부분만 열려 있습니다. 상자 안에는 실제 반려동물이 들어가 있으며, 머리와 턱 아래, 가슴 윗부분만 상자 위로 작게 빼꼼 보이게 해 주세요. 몸통과 등은 대부분 상자 안에 가려져 있어야 합니다. 표정은 시큰둥하거나 어리둥절한 자연스러운 반려동물 표정으로 해 주세요.\n\n상자 앞면 중앙에는 첨부한 반려동물과 동일한 반려동물을 작은 브릭 블록으로 정교하게 만든 완성품 이미지가 크게 인쇄되어 있어야 합니다. 단색 피규어가 아니라 실제 반려동물의 털색, 무늬, 얼굴 음영, 눈 색까지 여러 색 브릭으로 세밀하게 구현된 맞춤 피규어처럼 보여야 합니다. 하단에는 같은 브릭 반려동물의 정면, 45도, 옆모습 작은 이미지 3개를 넣어 주세요.\n\n상자 왼쪽 위에는 빨간 정사각형 안에 “TOY”라고 적힌 가상 로고를 넣어 주세요. 오른쪽 위와 측면에는 CUSTOM PET, BRICK PORTRAIT SERIES, 16+, 2480 PCS 같은 짧고 읽을 수 있는 영문 제품 정보를 자연스럽게 넣어 주세요. 상자 옆 바닥에는 반려동물 털색과 비슷한 작은 브릭 부품이 담긴 투명 비닐 봉투를 작게 놓아 주세요.\n\n카메라는 위에서 살짝 내려다보는 비스듬한 스마트폰 사진 구도로 해 주세요. 상자 앞면과 옆면, 열린 상자 날개가 함께 보여야 합니다. 배경은 밝은 중성톤 카펫이 깔린 집 안이며, 전체적으로 광고 화보보다 보호자가 우연히 찍은 귀엽고 현실적인 밈 사진 느낌으로 표현해 주세요.\n\n세로 4:5 비율, 사실적인 사진, 사람 금지, 반려동물 추가 생성 금지, 일반 택배 상자 느낌 금지, 워터마크·SNS 화면·실제 브랜드 로고 복제 금지.",
    "images": [
      {
        "src": "images/p029-01.png"
      }
    ]
  },
  {
    "id": "P-028",
    "animal": "dog",
    "title": "강아지 꽃다발",
    "category": "이미지",
    "cover": "images/p028-01.png",
    "description": "우리 강아지 사진을 넣어주세요",
    "prompt": "첨부한 반려동물 사진을 주인공으로 사용해 주세요. 사진 속 반려동물의 털색, 무늬, 얼굴형, 눈 색, 코와 입 모양, 귀 모양을 최대한 그대로 유지해 주세요. 반드시 사진 속 반려동물과 같은 반려동물처럼 보여야 합니다. 반려동물이 커다란 생화 꽃다발 한가운데에 폭 파묻혀 있는 귀엽고 사랑스러운 사진을 만들어 주세요. 꽃다발은 사람이 두 팔로 안아야 할 정도로 크고 풍성해야 하며, 반려동물의 머리와 목, 가슴 윗부분만 꽃 사이로 자연스럽게 드러나게 해 주세요. 반려동물이 꽃다발 앞에 따로 앉아 있거나 꽃 위에 올라탄 모습이 아니라, 실제로 꽃다발의 중심에 포근하게 들어가 얼굴만 빼꼼 내민 모습이어야 합니다. 반려동물 얼굴 주변을 크고 탐스러운 꽃들이 둥글게 감싸도록 배치해 주세요. 꽃잎이나 잎사귀가 눈과 코를 가리지 않게 하고, 얼굴은 이미지의 가장 중요한 중심으로 또렷하게 보여 주세요. 꽃의 색상은 한 가지 계열을 중심으로 통일해 주세요. 생성할 때마다 아래 색상 중 하나를 자연스럽게 선택해 주세요. 따뜻하고 화사한 노란색 꽃다발 부드럽고 사랑스러운 연분홍색 꽃다발 깨끗하고 청량한 하늘색과 흰색 꽃다발 우아한 크림색과 연보라색 꽃다발 꽃은 장미, 거베라, 카네이션, 작은 들꽃과 봉오리 등을 풍성하게 섞되, 지나치게 복잡하거나 빽빽하게 뭉개져 보이지 않도록 해 주세요. 각각의 꽃 형태가 자연스럽고 분명하게 보여야 합니다. 반려동물은 카메라를 정면으로 바라보며, 조금 놀란 듯 동그란 눈이나 얌전하고 웃는듯한 표정을 짓고 있습니다. 사람처럼 웃는 표정이나 과장된 인형 같은 표정은 피하고, 실제 반려동물 특유의 귀엽고 엉뚱한 분위기를 살려 주세요. 꽃다발은 고급스럽고 깔끔한 무광 포장지로 감싸고, 화면 아래쪽에는 꽃다발을 안고 있는 사람의 손이나 팔 일부만 자연스럽게 보이게 해 주세요. 사람의 얼굴과 몸 전체는 나오지 않게 해 주세요. 밝고 부드러운 자연광, 깨끗한 실내 배경, 따뜻하고 화사한 색감, 실제 스마트폰이나 카메라로 촬영한 듯한 선명한 반려동물 사진으로 표현해 주세요. 인스타그램 피드에 잘 어울리는 세로 4:5 비율로 만들어 주세요. 꽃다발과 반려동물이 화면을 풍성하게 채우도록 하고, 위아래 여백이나 테두리는 넣지 마세요. 글자, 로고, 워터마크, SNS 화면, 리본 문구는 넣지 마세요. 반려동물의 몸이 꽃으로 변하거나 꽃과 합쳐지지 않게 해 주세요. 꽃잎으로 된 귀, 꽃 모양 눈, 사람 손처럼 변한 앞발, 비정상적으로 많은 꽃잎, 찌그러진 꽃, 중복된 귀와 눈은 표현하지 마세요.",
    "images": [
      {
        "src": "images/p028-01.png"
      }
    ]
  },
  {
    "id": "P-027",
    "animal": "dog",
    "title": "강아지 꽃 화보",
    "category": "이미지",
    "cover": "images/p027-01.png",
    "description": "우리 강아지 사진을 넣어주세요",
    "prompt": "첨부한 반려동물 사진을 주인공으로 사용해 주세요.\n\n사진 속 반려동물의 털색, 무늬, 얼굴형, 눈 색, 귀 모양, 체형을 최대한 그대로 유지해 주세요.\n반드시 같은 반려동물처럼 보여야 합니다.\n\n밝고 화사한 야외 반려동물 화보 사진을 만들어 주세요.\n전체 분위기는 깨끗하고 맑고 따뜻한 봄~초여름 감성입니다.\n배경은 벽, 담장, 창문가, 조용한 골목 중 하나처럼 깔끔하고 단정한 공간으로 표현해 주세요.\n\n반려동물 주변에는 화사한 꽃이 풍성하게 피어 있어야 합니다.\n꽃은 연한 하늘색, 노란색, 분홍색 계열 중 하나로 표현하고,\n벽을 따라 흐르거나, 위에서 아래로 늘어지거나, 담장 주변을 감싸는 식으로 자연스럽게 배치해 주세요.\n\n반려동물의 포즈와 구도는 자연스러운 화보 느낌 안에서 조금씩 다르게 해 주세요.\n예를 들면:\n담장 위에 앞발을 올리고 앉아 있는 모습,\n벽에 기대어 편안히 누워 있는 모습,\n꽃 아래에서 눈을 감고 햇살을 즐기는 모습,\n창문 밖으로 고개를 내미는 모습,\n살짝 위를 올려다보는 모습 등으로 표현해 주세요.\n\n카메라 구도도 고정하지 말고, 클로즈업, 반신샷, 전신샷 중 자연스럽게 선택해 주세요.\n하지만 전체적으로는 깨끗하고 미니멀한 감성 화보 톤을 유지해 주세요.\n\n빛은 부드러운 자연광, 선명하지만 부드러운 그림자, 맑은 하늘, 은은한 아웃포커싱이 느껴지게 해 주세요.\n복잡한 소품은 넣지 말고, 감성적인 반려동물 화보처럼 자연스럽고 아름답게 표현해 주세요.\n\n중요한 점은 반려동물의 정체성과 전체 화보 분위기는 유지하되,\n꽃 색, 꽃 배치, 구도, 포즈는 생성할 때마다 조금씩 다르게 나오는 것입니다.\n\n세로형 이미지, 인스타그램 감성 화보 느낌으로 만들어 주세요. 필수 : AR 4:5",
    "images": [
      {
        "src": "images/p027-01.png"
      }
    ]
  },
  {
    "id": "P-026",
    "animal": "dog",
    "title": "계곡 콩이",
    "category": "이미지",
    "cover": "images/p026-01.png",
    "description": "우리 강아지 사진을 첨부해주세요",
    "prompt": "첨부한 반려동물 사진을 주인공으로 사용해 주세요. 사진 속 반려동물의 털색, 무늬, 얼굴형, 눈 색, 귀 모양과 체형을 그대로 유지해 주세요. 한여름의 맑고 시원한 계곡입니다. 반려동물이 아주 얕고 맑은 계곡물 위에 발라당 누워서 눈을 감고 편안하게 피서를 즐기고 있습니다. 반려동물은 몸의 일부가 얕은 물에 닿아 있고, 네 발은 힘을 뺀 채 자연스럽게 벌어져 있습니다. 표정은 매우 편안하고 나른하며, 눈을 감은 채 더위를 식히는 모습입니다. 배와 앞발, 뒷발이 귀엽게 드러나고, 온몸에 긴장이 풀린 듯한 자세로 누워 있습니다. 반려동물 주변의 물결은 아주 잔잔하게 퍼지고 있으며, 몸 주변에는 작은 물반사와 은은한 물결 표현이 있습니다. 투명한 계곡물 아래로 둥근 조약돌이 보이고, 주변에는 초록색 나뭇잎과 큰 바위가 놓여 있습니다. 여름 햇빛이 계곡물 위에서 반짝이고, 전체적으로 밝고 청량하며 평화로운 분위기로 표현해 주세요. 반려동물이 더 작고 귀엽게 보이도록 주변의 바위, 조약돌, 나뭇잎은 조금 크게 표현해 주세요. 전체 분위기는 유쾌하고 사랑스러운 여름 계곡 풍경입니다. 자연스럽고 선명한 이미지, 맑고 투명한 물, 귀엽고 편안한 반려동물의 모습이 잘 드러나게 해 주세요. 이미지 비율 4:5.",
    "images": [
      {
        "src": "images/p026-01.png"
      }
    ]
  },
  {
    "id": "P-025",
    "animal": "dog",
    "pinned": true,
    "title": "여름휴가 이모티콘",
    "category": "이모티콘",
    "cover": "images/p025-01.png",
    "description": "우리 강아지 사진을 첨부해주세요",
    "images": [
      {
        "src": "images/p025-01.png",
        "title": "여름엔 카페",
        "caption": "이미지마다 프롬프트가 달라요",
        "prompt": "첨부한 반려동물 사진과 반드시 같은 반려동물처럼 보이는 여름 카페 휴식 테마의 귀여운 한국식 이모티콘 스티커 시트를 만들어 주세요.\n반려동물의 털색, 줄무늬, 얼굴형, 눈, 코, 귀, 볼살, 체형, 목걸이를 최대한 그대로 유지해 주세요.\n16개의 스티커는 모두 같은 반려동물로 보이되, 복붙처럼 똑같아 보이지 않게 표정, 포즈, 문구, 소품을 다양하게 바꿔 주세요.\n\n전체 이미지는 상업용 스티커 시트 디자인처럼 깔끔하고 완성도 높게 표현해 주세요.\n16개의 개별 스티커를 4x4 배열로 배치하고, 각 스티커는 서로 겹치지 않게 적당한 간격을 두고 배치해 주세요.\n각 스티커에는 흰색 칼선 느낌의 외곽 테두리를 분명하게 넣어 주세요.\n\n중요하게, 흰색 외곽선이 잘 보이도록 전체 시트의 바탕 배경은 흰색이 아닌 아주 연한 베이지색 단색 배경으로 표현해 주세요.\n배경은 복잡한 무늬 없이 밝고 부드러운 연베이지 톤이어야 하며, 스티커가 잘 돋보이도록 깔끔하게 처리해 주세요.\n예시 느낌: 아주 연한 크림베이지 / 부드러운 종이 느낌의 밝은 베이지.\n\n각 스티커는 모두 서로 다른 표정, 포즈, 문구, 장식, 여름 소품을 가져야 합니다.\n아래 요소들은 매번 임의로 조합을 바꿔 새로운 세트처럼 구성해 주세요.\n\n표정 후보:\n설렘, 여유, 만족, 반짝이는 눈, 졸림, 장난기, 멍함, 신남, 시원해서 행복함, 사랑스러움\n\n포즈 후보:\n정면, 고개 갸웃, 디저트 옆에 앉기, 아이스 음료 옆에 앉기, 선글라스 쓰기, 두 앞발 모으기, 테이블 느낌의 소품 앞에 앉기, 수박 빙수 옆에 앉기, 빨대 컵 옆에 기대기, 낮잠 자기\n\n문구 후보:\n카페 왔어요!, 빙수 먹어요!, 아이스가 최고!, 오늘은 여유롭게, 디저트 타임!, 시원해요~, 엄마도 한입, 휴가 중이에요, 여름 간식 최고!, 사진 찍어 주세요!, 행복한 휴식, 놀 준비 끝!, 수박 주세요!, 너무 좋아요!, 쉬는 중이에요, 달달해요\n\n장식/소품 후보:\n아이스 음료, 빙수, 수박, 아이스크림, 빨대컵, 디저트 접시, 체리, 선글라스, 반짝이, 하트, 작은 파라솔, 민트잎, 부채, 얼음 조각, 카페 트레이\n\n전체 분위기는 귀엽고 밝고 선명한 상업용 스티커 느낌으로 표현해 주세요.\n각 스티커는 감정 표현이 잘 드러나고, 카카오톡 이모티콘처럼 직관적이고 사랑스럽게 만들어 주세요.\n문구는 각 스티커 아래 또는 어울리는 위치에 짧고 잘 읽히는 한국어 텍스트로 넣어 주세요.\n텍스트 색상은 스티커마다 어울리게 다양하게 적용하되, 전체적으로 통일감 있게 구성해 주세요.\n\n주의사항:\n같은 반려동물로 반드시 통일감 있게 표현할 것\n16개 모두 서로 다른 느낌으로 만들 것\n흰 칼선 외곽이 배경과 명확히 구분되도록 할 것\n실사 배경, 손, 책상, SNS UI, 그림자 효과 없이 오직 스티커 시트만 표현할 것\n완성 이미지는 스티커 상품 제작용 시안처럼 깔끔하고 선명하게 표현할 것\n\n[필수: AR 4:5]"
      },
      {
        "src": "images/p025-02.png",
        "title": "여름엔 방콕",
        "caption": "이미지마다 프롬프트가 달라요",
        "prompt": "첨부한 반려동물 사진과 반드시 같은 반려동물처럼 보이는 여름 집콕 피서 테마의 귀여운 한국식 이모티콘 스티커 시트를 만들어 주세요.\n반려동물의 털색, 줄무늬, 얼굴형, 눈, 코, 귀, 볼살, 체형, 목걸이를 최대한 그대로 유지해 주세요.\n16개의 스티커는 모두 같은 반려동물로 보이되, 복붙처럼 똑같아 보이지 않게 표정, 포즈, 문구, 소품을 다양하게 바꿔 주세요.\n\n전체 이미지는 상업용 스티커 시트 디자인처럼 깔끔하고 완성도 높게 표현해 주세요.\n16개의 개별 스티커를 4x4 배열로 배치하고, 각 스티커는 서로 겹치지 않게 적당한 간격을 두고 배치해 주세요.\n각 스티커에는 흰색 칼선 느낌의 외곽 테두리를 분명하게 넣어 주세요.\n\n중요하게, 흰색 외곽선이 잘 보이도록 전체 시트의 바탕 배경은 흰색이 아닌 아주 연한 베이지색 단색 배경으로 표현해 주세요.\n배경은 복잡한 무늬 없이 밝고 부드러운 연베이지 톤이어야 하며, 스티커가 잘 돋보이도록 깔끔하게 처리해 주세요.\n예시 느낌: 아주 연한 크림베이지 / 부드러운 종이 느낌의 밝은 베이지.\n\n각 스티커는 모두 서로 다른 표정, 포즈, 문구, 장식, 여름 소품을 가져야 합니다.\n아래 요소들은 매번 임의로 조합을 바꿔 새로운 세트처럼 구성해 주세요.\n\n표정 후보:\n더위에 지침, 시원해서 행복함, 멍함, 졸림, 여유, 만족, 반짝이는 눈, 장난기, 설렘, 늘어짐\n\n포즈 후보:\n정면, 고개 갸웃, 에어컨 바람 맞기, 선풍기 앞에 앉기, 배를 보이고 눕기, 쿠션 위에 앉기, 수박 옆에 앉기, 얼음물 옆에 앉기, 두 앞발 모으기, 낮잠 자기\n\n문구 후보:\n집이 최고!, 에어컨이 최고!, 더워서 녹아요, 시원해요~, 집콕 중이에요, 오늘은 안 나가요, 낮잠 타임!, 수박 먹어요!, 엄마도 쉬세요, 여기가 피서지!, 뒹굴뒹굴, 너무 좋아요!, 행복한 휴가, 시원한 집이 최고!, 침대에서 놀아요!, 귀찮아요\n\n장식/소품 후보:\n에어컨 바람 표시, 선풍기, 수박, 얼음물, 부채, 쿠션, 냉감방석 느낌 소품, 아이스크림, 반짝이, 하트, 작은 담요, 리모컨, 냉방 아이콘, 물방울, 체크 쿠션\n\n전체 분위기는 귀엽고 밝고 선명한 상업용 스티커 느낌으로 표현해 주세요.\n각 스티커는 감정 표현이 잘 드러나고, 카카오톡 이모티콘처럼 직관적이고 사랑스럽게 만들어 주세요.\n문구는 각 스티커 아래 또는 어울리는 위치에 짧고 잘 읽히는 한국어 텍스트로 넣어 주세요.\n텍스트 색상은 스티커마다 어울리게 다양하게 적용하되, 전체적으로 통일감 있게 구성해 주세요.\n\n주의사항:\n같은 반려동물로 반드시 통일감 있게 표현할 것\n16개 모두 서로 다른 느낌으로 만들 것\n흰 칼선 외곽이 배경과 명확히 구분되도록 할 것\n실사 배경, 손, 책상, SNS UI, 그림자 효과 없이 오직 스티커 시트만 표현할 것\n완성 이미지는 스티커 상품 제작용 시안처럼 깔끔하고 선명하게 표현할 것\n\n[필수: AR 4:5]"
      },
      {
        "src": "images/p025-03.png",
        "title": "여름엔 워터파크",
        "caption": "이미지마다 프롬프트가 달라요",
        "prompt": "첨부한 반려동물 사진과 반드시 같은 반려동물처럼 보이는 여름 워터파크 테마의 귀여운 한국식 이모티콘 스티커 시트를 만들어 주세요.\n반려동물의 털색, 줄무늬, 얼굴형, 눈, 코, 귀, 볼살, 체형, 목걸이를 최대한 그대로 유지해 주세요.\n16개의 스티커는 모두 같은 반려동물로 보이되, 복붙처럼 똑같아 보이지 않게 표정, 포즈, 문구, 소품을 다양하게 바꿔 주세요.\n\n전체 이미지는 상업용 스티커 시트 디자인처럼 깔끔하고 완성도 높게 표현해 주세요.\n16개의 개별 스티커를 4x4 배열로 배치하고, 각 스티커는 서로 겹치지 않게 적당한 간격을 두고 배치해 주세요.\n각 스티커에는 흰색 칼선 느낌의 외곽 테두리를 분명하게 넣어 주세요.\n\n중요하게, 흰색 외곽선이 잘 보이도록 전체 시트의 바탕 배경은 흰색이 아닌 아주 연한 베이지색 단색 배경으로 표현해 주세요.\n배경은 복잡한 무늬 없이 밝고 부드러운 연베이지 톤이어야 하며, 스티커가 잘 돋보이도록 깔끔하게 처리해 주세요.\n예시 느낌: 아주 연한 크림베이지 / 부드러운 종이 느낌의 밝은 베이지.\n\n각 스티커는 모두 서로 다른 표정, 포즈, 문구, 장식, 여름 소품을 가져야 합니다.\n아래 요소들은 매번 임의로 조합을 바꿔 새로운 세트처럼 구성해 주세요.\n\n표정 후보:\n신남, 설렘, 놀람, 더위에 지침, 시원해서 행복함, 반짝이는 눈, 장난기, 멍함, 여유, 졸림\n\n포즈 후보:\n정면, 고개 갸웃, 튜브 위에 앉기, 물장구 치기, 슬라이드 타기 직전 포즈, 선글라스 쓰기, 파라솔 아래 쉬기, 비치볼 옆에 앉기, 두 앞발 모으기, 낮잠 자기\n\n문구 후보:\n물놀이 갈까요?, 워터파크예요!, 튜브는 필수!, 너무 시원해요~, 오늘은 휴가!, 슬라이드 타요!, 더워서 녹아요, 엄마도 쉬세요, 행복한 휴가, 사진 찍어 주세요!, 놀 준비 끝!, 첨벙첨벙!, 수영장이 최고!, 시원해서 좋아요, 신나요!, 바캉스예요!\n\n장식/소품 후보:\n튜브, 물방울, 비치볼, 파라솔, 선글라스, 아이스크림, 수박, 부채, 반짝이, 하트, 파도, 슬라이드, 야자수, 구명조끼 느낌 소품, 물총, 시원한 음료\n\n전체 분위기는 귀엽고 밝고 선명한 상업용 스티커 느낌으로 표현해 주세요.\n각 스티커는 감정 표현이 잘 드러나고, 카카오톡 이모티콘처럼 직관적이고 사랑스럽게 만들어 주세요.\n문구는 각 스티커 아래 또는 어울리는 위치에 짧고 잘 읽히는 한국어 텍스트로 넣어 주세요.\n텍스트 색상은 스티커마다 어울리게 다양하게 적용하되, 전체적으로 통일감 있게 구성해 주세요.\n\n주의사항:\n같은 반려동물로 반드시 통일감 있게 표현할 것\n16개 모두 서로 다른 느낌으로 만들 것\n흰 칼선 외곽이 배경과 명확히 구분되도록 할 것\n실사 배경, 손, 책상, SNS UI, 그림자 효과 없이 오직 스티커 시트만 표현할 것\n완성 이미지는 스티커 상품 제작용 시안처럼 깔끔하고 선명하게 표현할 것\n\n[필수: AR 4:5]"
      },
      {
        "src": "images/p025-04.png",
        "title": "여름엔 바다",
        "caption": "이미지마다 프롬프트가 달라요",
        "prompt": "첨부한 반려동물 사진과 반드시 같은 반려동물처럼 보이는 여름휴가 테마의 귀여운 한국식 이모티콘 스티커 시트를 만들어 주세요.\n반려동물의 털색, 줄무늬, 얼굴형, 눈, 코, 귀, 볼살, 체형, 목걸이를 최대한 그대로 유지해 주세요.\n16개의 스티커는 모두 같은 반려동물로 보이되, 복붙처럼 똑같아 보이지 않게 표정, 포즈, 문구, 소품을 다양하게 바꿔 주세요.\n\n전체 이미지는 상업용 스티커 시트 디자인처럼 깔끔하고 완성도 높게 표현해 주세요.\n16개의 개별 스티커를 4x4 배열로 배치하고, 각 스티커는 서로 겹치지 않게 적당한 간격을 두고 배치해 주세요.\n각 스티커에는 흰색 칼선 느낌의 외곽 테두리를 분명하게 넣어 주세요.\n\n중요하게, 흰색 외곽선이 잘 보이도록 전체 시트의 바탕 배경은 흰색이 아닌 아주 연한 베이지색 단색 배경으로 표현해 주세요.\n배경은 복잡한 무늬 없이 밝고 부드러운 연베이지 톤이어야 하며, 스티커가 잘 돋보이도록 깔끔하게 처리해 주세요.\n예시 느낌: 아주 연한 크림베이지 / 부드러운 종이 느낌의 밝은 베이지.\n\n각 스티커는 모두 서로 다른 표정, 포즈, 문구, 장식, 여름 소품을 가져야 합니다.\n아래 요소들은 매번 임의로 조합을 바꿔 새로운 세트처럼 구성해 주세요.\n\n표정 후보:\n신남, 설렘, 더위에 지침, 시원해서 행복함, 졸림, 놀람, 반짝이는 눈, 장난기, 멍함, 여유\n\n포즈 후보:\n정면, 고개 갸웃, 앞발 들기, 두 앞발 모으기, 튜브 위에 앉기, 물장구 치기, 파라솔 아래 쉬기, 선글라스 쓰기, 수박 옆에 앉기, 낮잠 자기\n\n문구 후보:\n여름이에요!, 휴가 출발!, 바다 가요!, 너무 시원해요~, 수박 먹어요!, 튜브는 필수!, 오늘은 휴가!, 더워서 녹아요, 엄마도 쉬세요, 물놀이 갈까요?, 행복한 휴가, 낮잠 타임!, 떠나요!, 여행 중이에요, 놀 준비 끝!, 사진 찍어 주세요!\n\n장식/소품 후보:\n햇빛, 파도, 물방울, 조개, 불가사리, 야자수, 수박, 아이스크림, 비치볼, 선글라스, 부채, 여행 가방, 파라솔, 반짝이, 하트\n\n전체 분위기는 귀엽고 밝고 선명한 상업용 스티커 느낌으로 표현해 주세요.\n각 스티커는 감정 표현이 잘 드러나고, 카카오톡 이모티콘처럼 직관적이고 사랑스럽게 만들어 주세요.\n문구는 각 스티커 아래 또는 어울리는 위치에 짧고 잘 읽히는 한국어 텍스트로 넣어 주세요.\n텍스트 색상은 스티커마다 어울리게 다양하게 적용하되, 전체적으로 통일감 있게 구성해 주세요.\n\n주의사항:\n\n같은 반려동물로 반드시 통일감 있게 표현할 것\n16개 모두 서로 다른 느낌으로 만들 것\n흰 칼선 외곽이 배경과 명확히 구분되도록 할 것\n실사 배경, 손, 책상, SNS UI, 그림자 효과 없이 오직 스티커 시트만 표현할 것\n완성 이미지는 스티커 상품 제작용 시안처럼 깔끔하고 선명하게 표현할 것\n\n[필수: AR 4:5]"
      },
      {
        "src": "images/p025-05.png",
        "title": "여름엔 호캉스",
        "caption": "이미지마다 프롬프트가 달라요",
        "prompt": "첨부한 반려동물 사진과 반드시 같은 반려동물처럼 보이는 여름 호캉스 테마의 귀여운 한국식 이모티콘 스티커 시트를 만들어 주세요.\n반려동물의 털색, 줄무늬, 얼굴형, 눈, 코, 귀, 볼살, 체형, 목걸이를 최대한 그대로 유지해 주세요.\n16개의 스티커는 모두 같은 반려동물로 보이되, 복붙처럼 똑같아 보이지 않게 표정, 포즈, 문구, 소품을 다양하게 바꿔 주세요.\n\n전체 이미지는 상업용 스티커 시트 디자인처럼 깔끔하고 완성도 높게 표현해 주세요.\n16개의 개별 스티커를 4x4 배열로 배치하고, 각 스티커는 서로 겹치지 않게 적당한 간격을 두고 배치해 주세요.\n각 스티커에는 흰색 칼선 느낌의 외곽 테두리를 분명하게 넣어 주세요.\n\n중요하게, 흰색 외곽선이 잘 보이도록 전체 시트의 바탕 배경은 흰색이 아닌 아주 연한 베이지색 단색 배경으로 표현해 주세요.\n배경은 복잡한 무늬 없이 밝고 부드러운 연베이지 톤이어야 하며, 스티커가 잘 돋보이도록 깔끔하게 처리해 주세요.\n예시 느낌: 아주 연한 크림베이지 / 부드러운 종이 느낌의 밝은 베이지.\n\n각 스티커는 모두 서로 다른 표정, 포즈, 문구, 장식, 여름 소품을 가져야 합니다.\n아래 요소들은 매번 임의로 조합을 바꿔 새로운 세트처럼 구성해 주세요.\n\n표정 후보:\n신남, 설렘, 여유, 졸림, 만족, 반짝이는 눈, 장난기, 멍함, 시원해서 행복함, 뿌듯함\n\n포즈 후보:\n정면, 고개 갸웃, 침대에 누워 있기, 베개 끌어안기, 창가에 앉기, 룸서비스 기다리기, 수건 두르고 있기, 선글라스 쓰기, 수영장 튜브 위에 앉기, 낮잠 자기\n\n문구 후보:\n호캉스예요!, 오늘은 쉬는 날!, 침대가 최고!, 룸서비스 주세요!, 시원해요~, 수영장 가요!, 엄마도 쉬세요, 낮잠 타임!, 행복한 휴가, 체크인 완료!, 뒹굴뒹굴, 사진 찍어 주세요!, 여행 중이에요, 너무 좋아요!, 놀 준비 끝!, 여유 만끽!\n\n장식/소품 후보:\n베개, 침대, 수건, 욕실 가운, 선글라스, 여행 가방, 키카드, 미니 수영장 튜브, 트레이, 음료, 아이스크림, 파라솔, 반짝이, 하트, 창문, 야자수 화분\n\n전체 분위기는 귀엽고 밝고 선명한 상업용 스티커 느낌으로 표현해 주세요.\n각 스티커는 감정 표현이 잘 드러나고, 카카오톡 이모티콘처럼 직관적이고 사랑스럽게 만들어 주세요.\n문구는 각 스티커 아래 또는 어울리는 위치에 짧고 잘 읽히는 한국어 텍스트로 넣어 주세요.\n텍스트 색상은 스티커마다 어울리게 다양하게 적용하되, 전체적으로 통일감 있게 구성해 주세요.\n\n주의사항:\n같은 반려동물로 반드시 통일감 있게 표현할 것\n16개 모두 서로 다른 느낌으로 만들 것\n흰 칼선 외곽이 배경과 명확히 구분되도록 할 것\n실사 배경, 손, 책상, SNS UI, 그림자 효과 없이 오직 스티커 시트만 표현할 것\n완성 이미지는 스티커 상품 제작용 시안처럼 깔끔하고 선명하게 표현할 것\n\n[필수: AR 4:5]"
      },
      {
        "src": "images/p025-06.png",
        "title": "여름엔 계곡",
        "caption": "이미지마다 프롬프트가 달라요",
        "prompt": "첨부한 반려동물 사진과 반드시 같은 반려동물처럼 보이는 여름 계곡 피서 테마의 귀여운 한국식 이모티콘 스티커 시트를 만들어 주세요.\n반려동물의 털색, 줄무늬, 얼굴형, 눈, 코, 귀, 볼살, 체형, 목걸이를 최대한 그대로 유지해 주세요.\n16개의 스티커는 모두 같은 반려동물로 보이되, 복붙처럼 똑같아 보이지 않게 표정, 포즈, 문구, 소품을 다양하게 바꿔 주세요.\n\n전체 이미지는 상업용 스티커 시트 디자인처럼 깔끔하고 완성도 높게 표현해 주세요.\n16개의 개별 스티커를 4x4 배열로 배치하고, 각 스티커는 서로 겹치지 않게 적당한 간격을 두고 배치해 주세요.\n각 스티커에는 흰색 칼선 느낌의 외곽 테두리를 분명하게 넣어 주세요.\n\n중요하게, 흰색 외곽선이 잘 보이도록 전체 시트의 바탕 배경은 흰색이 아닌 아주 연한 베이지색 단색 배경으로 표현해 주세요.\n배경은 복잡한 무늬 없이 밝고 부드러운 연베이지 톤이어야 하며, 스티커가 잘 돋보이도록 깔끔하게 처리해 주세요.\n예시 느낌: 아주 연한 크림베이지 / 부드러운 종이 느낌의 밝은 베이지.\n\n각 스티커는 모두 서로 다른 표정, 포즈, 문구, 장식, 여름 소품을 가져야 합니다.\n아래 요소들은 매번 임의로 조합을 바꿔 새로운 세트처럼 구성해 주세요.\n\n표정 후보:\n신남, 설렘, 시원해서 행복함, 더위에 지침, 졸림, 놀람, 반짝이는 눈, 장난기, 멍함, 여유\n\n포즈 후보:\n정면, 고개 갸웃, 앞발 들기, 두 앞발 모으기, 계곡물에 발 담그기, 물장구 치기, 바위 위에 앉기, 그늘 아래 쉬기, 수박 옆에 앉기, 낮잠 자기\n\n문구 후보:\n계곡 왔어요!, 물이 시원해요~, 발 담그는 중이에요, 여기가 천국!, 수박 먹어요!, 오늘은 피서!, 더위 탈출!, 물놀이 최고!, 엄마도 쉬세요, 바위 명당이에요, 낮잠 타임!, 풍경이 좋아요, 놀 준비 끝!, 사진 찍어 주세요!, 행복한 피서, 물소리가 좋아요\n\n장식/소품 후보:\n계곡물, 물방울, 바위, 나뭇잎, 수박, 부채, 반짝이, 하트, 조약돌, 작은 폭포, 그늘, 대나무 바구니, 튜브, 아이스 음료, 발수건\n\n전체 분위기는 귀엽고 밝고 선명한 상업용 스티커 느낌으로 표현해 주세요.\n각 스티커는 감정 표현이 잘 드러나고, 카카오톡 이모티콘처럼 직관적이고 사랑스럽게 만들어 주세요.\n문구는 각 스티커 아래 또는 어울리는 위치에 짧고 잘 읽히는 한국어 텍스트로 넣어 주세요.\n텍스트 색상은 스티커마다 어울리게 다양하게 적용하되, 전체적으로 통일감 있게 구성해 주세요.\n\n주의사항:\n같은 반려동물로 반드시 통일감 있게 표현할 것\n16개 모두 서로 다른 느낌으로 만들 것\n흰 칼선 외곽이 배경과 명확히 구분되도록 할 것\n실사 배경, 손, 책상, SNS UI, 그림자 효과 없이 오직 스티커 시트만 표현할 것\n완성 이미지는 스티커 상품 제작용 시안처럼 깔끔하고 선명하게 표현할 것\n\n[필수: AR 4:5]"
      }
    ]
  },
  {
    "id": "P-024",
    "title": "손가락 끝 요정냥",
    "category": "귀여운 이미지",
    "cover": "images/p024-01.png",
    "description": "만들때마다 랜덤 코스튬~!",
    "prompt": "첨부한 고양이 사진을 사용해 주세요. 사진 속 고양이의 털색, 무늬, 얼굴형, 눈, 코, 입, 귀, 볼살, 체형을 최대한 그대로 유지해 주세요. 반드시 실제 사진과 같은 고양이처럼 보여야 합니다.\n\n고양이가 사람의 손가락 끝 위에 올라간 미니 사이즈로 표현된 귀엽고 사실적인 사진을 만들어 주세요. 고양이는 손가락의 중간 부분이 아니라 반드시 손가락 맨 끝의 둥근 부분 위에 올라가 있어야 합니다. 너무 작게 점처럼 보이지 않도록 해 주세요. 손가락 끝 위에서 존재감 있게 보이되, 여전히 “손가락 위에 올라간 작은 고양이”처럼 느껴져야 합니다. 고양이 몸집은 손가락 끝 너비의 약 60~75% 정도로 보이게 해 주세요. 손가락 피부가 고양이 주변으로 자연스럽게 충분히 보이도록 해 주세요.\n\nAdditional handling notes: determine a single costume variation by using the current local time at the moment this request is processed. Read the time as a 24-hour HHMM value, add the four digits together, and use the remainder when divided by 10 to choose exactly one costume. Do not explain the rule, do not mention the calculation, and do not output any text about the selection. Apply only one costume, never mix multiple costumes. Use this mapping: 0 bee costume, 1 bunny costume, 2 chick costume, 3 frog raincoat costume, 4 strawberry costume, 5 astronaut costume, 6 wizard costume, 7 angel costume, 8 prince or princess costume, 9 shark hood costume. Also make the facial mood and atmosphere naturally match the selected costume.\n\n고양이는 손가락 끝 위에 안정감 있게 앉아 있거나 앞발을 모으고 살짝 기대 앉은 자세로 표현해 주세요. 고양이에게는 귀엽고 작은 코스튬 또는 테마 의상 1종만 적용해 주세요. 의상은 봉제 인형 코스튬처럼 사랑스럽고 아기자기하게 표현해 주세요. 고양이 얼굴은 최대한 선명하고 매력적으로 보여야 하며, 손가락과 고양이의 크기 대비가 한눈에 느껴져야 합니다.\n\n배경은 깔끔하고 자연스럽게 흐려진 밝은 실내 또는 중성 배경으로 해 주세요. 전체 이미지는 스마트폰으로 찍은 듯한 사실적인 느낌으로 표현해 주세요. 세로 4:5 비율, 여백 없이, 글자, 말풍선, 워터마크, SNS 화면 요소 없이 표현해 주세요.",
    "images": [
      {
        "src": "images/p024-01.png"
      },
      {
        "src": "images/p024-02.png"
      },
      {
        "src": "images/p024-03.png"
      },
      {
        "src": "images/p024-04.png"
      },
      {
        "src": "images/p024-05.png"
      },
      {
        "src": "images/p024-06.png"
      },
      {
        "src": "images/p024-07.png"
      },
      {
        "src": "images/p024-08.png"
      },
      {
        "src": "images/p024-09.png"
      },
      {
        "src": "images/p024-10.png"
      }
    ],
    "animal": "cat"
  },
  {
    "id": "P-023",
    "title": "여름 휴가 냥모티콘 시리즈",
    "category": "계절",
    "cover": "images/p023-01.png",
    "description": "이미지마다 프롬프트가 달라요",
    "images": [
      {
        "src": "images/p023-01.png",
        "title": "피서는 카페냥",
        "caption": "원하는 버전의 프롬프트를 복사하세요",
        "prompt": "첨부한 고양이 사진과 반드시 같은 고양이처럼 보이는 여름 카페 휴식 테마의 귀여운 한국식 이모티콘 스티커 시트를 만들어 주세요.\n고양이의 털색, 줄무늬, 얼굴형, 눈, 코, 귀, 볼살, 체형, 목걸이를 최대한 그대로 유지해 주세요.\n16개의 스티커는 모두 같은 고양이로 보이되, 복붙처럼 똑같아 보이지 않게 표정, 포즈, 문구, 소품을 다양하게 바꿔 주세요.\n\n전체 이미지는 상업용 스티커 시트 디자인처럼 깔끔하고 완성도 높게 표현해 주세요.\n16개의 개별 스티커를 4x4 배열로 배치하고, 각 스티커는 서로 겹치지 않게 적당한 간격을 두고 배치해 주세요.\n각 스티커에는 흰색 칼선 느낌의 외곽 테두리를 분명하게 넣어 주세요.\n\n중요하게, 흰색 외곽선이 잘 보이도록 전체 시트의 바탕 배경은 흰색이 아닌 아주 연한 베이지색 단색 배경으로 표현해 주세요.\n배경은 복잡한 무늬 없이 밝고 부드러운 연베이지 톤이어야 하며, 스티커가 잘 돋보이도록 깔끔하게 처리해 주세요.\n예시 느낌: 아주 연한 크림베이지 / 부드러운 종이 느낌의 밝은 베이지.\n\n각 스티커는 모두 서로 다른 표정, 포즈, 문구, 장식, 여름 소품을 가져야 합니다.\n아래 요소들은 매번 임의로 조합을 바꿔 새로운 세트처럼 구성해 주세요.\n\n표정 후보:\n설렘, 여유, 만족, 반짝이는 눈, 졸림, 장난기, 멍함, 신남, 시원해서 행복함, 사랑스러움\n\n포즈 후보:\n정면, 고개 갸웃, 디저트 옆에 앉기, 아이스 음료 옆에 앉기, 선글라스 쓰기, 두 앞발 모으기, 테이블 느낌의 소품 앞에 앉기, 수박 빙수 옆에 앉기, 빨대 컵 옆에 기대기, 낮잠 자기\n\n문구 후보:\n카페 왔다냥!, 빙수 먹자냥, 아이스 최고냥, 오늘은 여유냥, 디저트 타임냥, 시원하냥~, 집사도 한입냥, 휴가 중이다냥, 여름 간식냥, 사진 찍어라냥, 행복한 휴식냥, 놀 준비 끝냥, 수박 주라냥, 너무 좋다냥, 쉬는 중이다냥, 달달하냥\n\n장식/소품 후보:\n아이스 음료, 빙수, 수박, 아이스크림, 빨대컵, 디저트 접시, 체리, 선글라스, 반짝이, 하트, 작은 파라솔, 민트잎, 부채, 얼음 조각, 카페 트레이\n\n전체 분위기는 귀엽고 밝고 선명한 상업용 스티커 느낌으로 표현해 주세요.\n각 스티커는 감정 표현이 잘 드러나고, 카카오톡 이모티콘처럼 직관적이고 사랑스럽게 만들어 주세요.\n문구는 각 스티커 아래 또는 어울리는 위치에 짧고 잘 읽히는 한국어 텍스트로 넣어 주세요.\n텍스트 색상은 스티커마다 어울리게 다양하게 적용하되, 전체적으로 통일감 있게 구성해 주세요.\n\n주의사항:\n같은 고양이로 반드시 통일감 있게 표현할 것\n16개 모두 서로 다른 느낌으로 만들 것\n흰 칼선 외곽이 배경과 명확히 구분되도록 할 것\n실사 배경, 손, 책상, SNS UI, 그림자 효과 없이 오직 스티커 시트만 표현할 것\n완성 이미지는 스티커 상품 제작용 시안처럼 깔끔하고 선명하게 표현할 것\n\n[필수: AR 4:5]"
      },
      {
        "src": "images/p023-02.png",
        "title": "피서는 바다냥",
        "caption": "원하는 버전의 프롬프트를 복사하세요",
        "prompt": "첨부한 고양이 사진과 반드시 같은 고양이처럼 보이는 여름휴가 테마의 귀여운 한국식 이모티콘 스티커 시트를 만들어 주세요.\n고양이의 털색, 줄무늬, 얼굴형, 눈, 코, 귀, 볼살, 체형, 목걸이를 최대한 그대로 유지해 주세요.\n16개의 스티커는 모두 같은 고양이로 보이되, 복붙처럼 똑같아 보이지 않게 표정, 포즈, 문구, 소품을 다양하게 바꿔 주세요.\n\n전체 이미지는 상업용 스티커 시트 디자인처럼 깔끔하고 완성도 높게 표현해 주세요.\n16개의 개별 스티커를 4x4 배열로 배치하고, 각 스티커는 서로 겹치지 않게 적당한 간격을 두고 배치해 주세요.\n각 스티커에는 흰색 칼선 느낌의 외곽 테두리를 분명하게 넣어 주세요.\n\n중요하게, 흰색 외곽선이 잘 보이도록 전체 시트의 바탕 배경은 흰색이 아닌 아주 연한 베이지색 단색 배경으로 표현해 주세요.\n배경은 복잡한 무늬 없이 밝고 부드러운 연베이지 톤이어야 하며, 스티커가 잘 돋보이도록 깔끔하게 처리해 주세요.\n예시 느낌: 아주 연한 크림베이지 / 부드러운 종이 느낌의 밝은 베이지.\n\n각 스티커는 모두 서로 다른 표정, 포즈, 문구, 장식, 여름 소품을 가져야 합니다.\n아래 요소들은 매번 임의로 조합을 바꿔 새로운 세트처럼 구성해 주세요.\n\n표정 후보:\n신남, 설렘, 더위에 지침, 시원해서 행복함, 졸림, 놀람, 반짝이는 눈, 장난기, 멍함, 여유\n\n포즈 후보:\n정면, 고개 갸웃, 앞발 들기, 두 앞발 모으기, 튜브 위에 앉기, 물장구 치기, 파라솔 아래 쉬기, 선글라스 쓰기, 수박 옆에 앉기, 낮잠 자기\n\n문구 후보:\n여름이다냥!, 휴가 출발냥, 바다 가자냥, 너무 시원하냥~, 수박 먹자냥, 튜브 필수냥, 오늘은 휴가냥, 더워서 녹냥, 집사도 쉬라냥, 물놀이 가냥?, 행복한 휴가냥, 낮잠 타임냥, 떠나자냥, 여행 중이다냥, 놀 준비 끝냥, 사진 찍어라냥\n\n장식/소품 후보:\n햇빛, 파도, 물방울, 조개, 불가사리, 야자수, 수박, 아이스크림, 비치볼, 선글라스, 부채, 여행 가방, 파라솔, 반짝이, 하트\n\n전체 분위기는 귀엽고 밝고 선명한 상업용 스티커 느낌으로 표현해 주세요.\n각 스티커는 감정 표현이 잘 드러나고, 카카오톡 이모티콘처럼 직관적이고 사랑스럽게 만들어 주세요.\n문구는 각 스티커 아래 또는 어울리는 위치에 짧고 잘 읽히는 한국어 텍스트로 넣어 주세요.\n텍스트 색상은 스티커마다 어울리게 다양하게 적용하되, 전체적으로 통일감 있게 구성해 주세요.\n\n주의사항:\n\n같은 고양이로 반드시 통일감 있게 표현할 것\n16개 모두 서로 다른 느낌으로 만들 것\n흰 칼선 외곽이 배경과 명확히 구분되도록 할 것\n실사 배경, 손, 책상, SNS UI, 그림자 효과 없이 오직 스티커 시트만 표현할 것\n완성 이미지는 스티커 상품 제작용 시안처럼 깔끔하고 선명하게 표현할 것\n\n[필수: AR 4:5]"
      },
      {
        "src": "images/p023-03.png",
        "title": "피서는 워터파크냥",
        "caption": "원하는 버전의 프롬프트를 복사하세요",
        "prompt": "첨부한 고양이 사진과 반드시 같은 고양이처럼 보이는 여름 워터파크 테마의 귀여운 한국식 이모티콘 스티커 시트를 만들어 주세요.\n고양이의 털색, 줄무늬, 얼굴형, 눈, 코, 귀, 볼살, 체형, 목걸이를 최대한 그대로 유지해 주세요.\n16개의 스티커는 모두 같은 고양이로 보이되, 복붙처럼 똑같아 보이지 않게 표정, 포즈, 문구, 소품을 다양하게 바꿔 주세요.\n\n전체 이미지는 상업용 스티커 시트 디자인처럼 깔끔하고 완성도 높게 표현해 주세요.\n16개의 개별 스티커를 4x4 배열로 배치하고, 각 스티커는 서로 겹치지 않게 적당한 간격을 두고 배치해 주세요.\n각 스티커에는 흰색 칼선 느낌의 외곽 테두리를 분명하게 넣어 주세요.\n\n중요하게, 흰색 외곽선이 잘 보이도록 전체 시트의 바탕 배경은 흰색이 아닌 아주 연한 베이지색 단색 배경으로 표현해 주세요.\n배경은 복잡한 무늬 없이 밝고 부드러운 연베이지 톤이어야 하며, 스티커가 잘 돋보이도록 깔끔하게 처리해 주세요.\n예시 느낌: 아주 연한 크림베이지 / 부드러운 종이 느낌의 밝은 베이지.\n\n각 스티커는 모두 서로 다른 표정, 포즈, 문구, 장식, 여름 소품을 가져야 합니다.\n아래 요소들은 매번 임의로 조합을 바꿔 새로운 세트처럼 구성해 주세요.\n\n표정 후보:\n신남, 설렘, 놀람, 더위에 지침, 시원해서 행복함, 반짝이는 눈, 장난기, 멍함, 여유, 졸림\n\n포즈 후보:\n정면, 고개 갸웃, 튜브 위에 앉기, 물장구 치기, 슬라이드 타기 직전 포즈, 선글라스 쓰기, 파라솔 아래 쉬기, 비치볼 옆에 앉기, 두 앞발 모으기, 낮잠 자기\n\n문구 후보:\n물놀이 가냥?, 워터파크다냥!, 튜브 필수냥, 너무 시원하냥~, 오늘은 휴가냥, 슬라이드 타냥!, 더워서 녹냥, 집사도 쉬라냥, 행복한 휴가냥, 사진 찍어라냥, 놀 준비 끝냥, 첨벙첨벙냥, 수영장 최고냥, 시원해서 좋냥, 신난다냥, 바캉스다냥\n\n장식/소품 후보:\n튜브, 물방울, 비치볼, 파라솔, 선글라스, 아이스크림, 수박, 부채, 반짝이, 하트, 파도, 슬라이드, 야자수, 구명조끼 느낌 소품, 물총, 시원한 음료\n\n전체 분위기는 귀엽고 밝고 선명한 상업용 스티커 느낌으로 표현해 주세요.\n각 스티커는 감정 표현이 잘 드러나고, 카카오톡 이모티콘처럼 직관적이고 사랑스럽게 만들어 주세요.\n문구는 각 스티커 아래 또는 어울리는 위치에 짧고 잘 읽히는 한국어 텍스트로 넣어 주세요.\n텍스트 색상은 스티커마다 어울리게 다양하게 적용하되, 전체적으로 통일감 있게 구성해 주세요.\n\n주의사항:\n같은 고양이로 반드시 통일감 있게 표현할 것\n16개 모두 서로 다른 느낌으로 만들 것\n흰 칼선 외곽이 배경과 명확히 구분되도록 할 것\n실사 배경, 손, 책상, SNS UI, 그림자 효과 없이 오직 스티커 시트만 표현할 것\n완성 이미지는 스티커 상품 제작용 시안처럼 깔끔하고 선명하게 표현할 것\n\n[필수: AR 4:5]"
      },
      {
        "src": "images/p023-04.png",
        "title": "피서는 집이다냥",
        "caption": "원하는 버전의 프롬프트를 복사하세요",
        "prompt": "첨부한 고양이 사진과 반드시 같은 고양이처럼 보이는 여름 집콕 피서 테마의 귀여운 한국식 이모티콘 스티커 시트를 만들어 주세요.\n고양이의 털색, 줄무늬, 얼굴형, 눈, 코, 귀, 볼살, 체형, 목걸이를 최대한 그대로 유지해 주세요.\n16개의 스티커는 모두 같은 고양이로 보이되, 복붙처럼 똑같아 보이지 않게 표정, 포즈, 문구, 소품을 다양하게 바꿔 주세요.\n\n전체 이미지는 상업용 스티커 시트 디자인처럼 깔끔하고 완성도 높게 표현해 주세요.\n16개의 개별 스티커를 4x4 배열로 배치하고, 각 스티커는 서로 겹치지 않게 적당한 간격을 두고 배치해 주세요.\n각 스티커에는 흰색 칼선 느낌의 외곽 테두리를 분명하게 넣어 주세요.\n\n중요하게, 흰색 외곽선이 잘 보이도록 전체 시트의 바탕 배경은 흰색이 아닌 아주 연한 베이지색 단색 배경으로 표현해 주세요.\n배경은 복잡한 무늬 없이 밝고 부드러운 연베이지 톤이어야 하며, 스티커가 잘 돋보이도록 깔끔하게 처리해 주세요.\n예시 느낌: 아주 연한 크림베이지 / 부드러운 종이 느낌의 밝은 베이지.\n\n각 스티커는 모두 서로 다른 표정, 포즈, 문구, 장식, 여름 소품을 가져야 합니다.\n아래 요소들은 매번 임의로 조합을 바꿔 새로운 세트처럼 구성해 주세요.\n\n표정 후보:\n더위에 지침, 시원해서 행복함, 멍함, 졸림, 여유, 만족, 반짝이는 눈, 장난기, 설렘, 늘어짐\n\n포즈 후보:\n정면, 고개 갸웃, 에어컨 바람 맞기, 선풍기 앞에 앉기, 배를 보이고 눕기, 쿠션 위에 앉기, 수박 옆에 앉기, 얼음물 옆에 앉기, 두 앞발 모으기, 낮잠 자기\n\n문구 후보:\n집이 최고냥, 에어컨 최고냥, 더워서 녹냥, 시원하냥~, 집콕 중이다냥, 오늘은 안 나간냥, 낮잠 타임냥, 수박 먹자냥, 집사도 쉬라냥, 여기가 피서지냥, 뒹굴뒹굴냥, 너무 좋다냥, 행복한 휴가냥, 시원한 집냥, 놀 준비는 침대냥, 귀찮냥\n\n장식/소품 후보:\n에어컨 바람 표시, 선풍기, 수박, 얼음물, 부채, 쿠션, 냉감방석 느낌 소품, 아이스크림, 반짝이, 하트, 작은 담요, 리모컨, 냉방 아이콘, 물방울, 체크 쿠션\n\n전체 분위기는 귀엽고 밝고 선명한 상업용 스티커 느낌으로 표현해 주세요.\n각 스티커는 감정 표현이 잘 드러나고, 카카오톡 이모티콘처럼 직관적이고 사랑스럽게 만들어 주세요.\n문구는 각 스티커 아래 또는 어울리는 위치에 짧고 잘 읽히는 한국어 텍스트로 넣어 주세요.\n텍스트 색상은 스티커마다 어울리게 다양하게 적용하되, 전체적으로 통일감 있게 구성해 주세요.\n\n주의사항:\n같은 고양이로 반드시 통일감 있게 표현할 것\n16개 모두 서로 다른 느낌으로 만들 것\n흰 칼선 외곽이 배경과 명확히 구분되도록 할 것\n실사 배경, 손, 책상, SNS UI, 그림자 효과 없이 오직 스티커 시트만 표현할 것\n완성 이미지는 스티커 상품 제작용 시안처럼 깔끔하고 선명하게 표현할 것\n\n[필수: AR 4:5]"
      },
      {
        "src": "images/p023-05.png",
        "title": "피서는 계곡냥",
        "caption": "원하는 버전의 프롬프트를 복사하세요",
        "prompt": "첨부한 고양이 사진과 반드시 같은 고양이처럼 보이는 여름 계곡 피서 테마의 귀여운 한국식 이모티콘 스티커 시트를 만들어 주세요.\n고양이의 털색, 줄무늬, 얼굴형, 눈, 코, 귀, 볼살, 체형, 목걸이를 최대한 그대로 유지해 주세요.\n16개의 스티커는 모두 같은 고양이로 보이되, 복붙처럼 똑같아 보이지 않게 표정, 포즈, 문구, 소품을 다양하게 바꿔 주세요.\n\n전체 이미지는 상업용 스티커 시트 디자인처럼 깔끔하고 완성도 높게 표현해 주세요.\n16개의 개별 스티커를 4x4 배열로 배치하고, 각 스티커는 서로 겹치지 않게 적당한 간격을 두고 배치해 주세요.\n각 스티커에는 흰색 칼선 느낌의 외곽 테두리를 분명하게 넣어 주세요.\n\n중요하게, 흰색 외곽선이 잘 보이도록 전체 시트의 바탕 배경은 흰색이 아닌 아주 연한 베이지색 단색 배경으로 표현해 주세요.\n배경은 복잡한 무늬 없이 밝고 부드러운 연베이지 톤이어야 하며, 스티커가 잘 돋보이도록 깔끔하게 처리해 주세요.\n예시 느낌: 아주 연한 크림베이지 / 부드러운 종이 느낌의 밝은 베이지.\n\n각 스티커는 모두 서로 다른 표정, 포즈, 문구, 장식, 여름 소품을 가져야 합니다.\n아래 요소들은 매번 임의로 조합을 바꿔 새로운 세트처럼 구성해 주세요.\n\n표정 후보:\n신남, 설렘, 시원해서 행복함, 더위에 지침, 졸림, 놀람, 반짝이는 눈, 장난기, 멍함, 여유\n\n포즈 후보:\n정면, 고개 갸웃, 앞발 들기, 두 앞발 모으기, 계곡물에 발 담그기, 물장구 치기, 바위 위에 앉기, 그늘 아래 쉬기, 수박 옆에 앉기, 낮잠 자기\n\n문구 후보:\n계곡 왔다냥!, 물이 시원하냥~, 발 담그는 중냥, 여기가 천국이냥, 수박 먹자냥, 오늘은 피서냥, 더위 탈출냥, 물놀이 최고냥, 집사도 쉬라냥, 바위 명당이다냥, 낮잠 타임냥, 풍경 좋다냥, 놀 준비 끝냥, 사진 찍어라냥, 행복한 피서냥, 물소리 좋냥\n\n장식/소품 후보:\n계곡물, 물방울, 바위, 나뭇잎, 수박, 부채, 반짝이, 하트, 조약돌, 작은 폭포, 그늘, 대나무 바구니, 튜브, 아이스 음료, 발수건\n\n전체 분위기는 귀엽고 밝고 선명한 상업용 스티커 느낌으로 표현해 주세요.\n각 스티커는 감정 표현이 잘 드러나고, 카카오톡 이모티콘처럼 직관적이고 사랑스럽게 만들어 주세요.\n문구는 각 스티커 아래 또는 어울리는 위치에 짧고 잘 읽히는 한국어 텍스트로 넣어 주세요.\n텍스트 색상은 스티커마다 어울리게 다양하게 적용하되, 전체적으로 통일감 있게 구성해 주세요.\n\n주의사항:\n같은 고양이로 반드시 통일감 있게 표현할 것\n16개 모두 서로 다른 느낌으로 만들 것\n흰 칼선 외곽이 배경과 명확히 구분되도록 할 것\n실사 배경, 손, 책상, SNS UI, 그림자 효과 없이 오직 스티커 시트만 표현할 것\n완성 이미지는 스티커 상품 제작용 시안처럼 깔끔하고 선명하게 표현할 것\n\n[필수: AR 4:5]"
      },
      {
        "src": "images/p023-06.png",
        "title": "피서는 호캉스냥",
        "caption": "원하는 버전의 프롬프트를 복사하세요",
        "prompt": "첨부한 고양이 사진과 반드시 같은 고양이처럼 보이는 여름 호캉스 테마의 귀여운 한국식 이모티콘 스티커 시트를 만들어 주세요.\n고양이의 털색, 줄무늬, 얼굴형, 눈, 코, 귀, 볼살, 체형, 목걸이를 최대한 그대로 유지해 주세요.\n16개의 스티커는 모두 같은 고양이로 보이되, 복붙처럼 똑같아 보이지 않게 표정, 포즈, 문구, 소품을 다양하게 바꿔 주세요.\n\n전체 이미지는 상업용 스티커 시트 디자인처럼 깔끔하고 완성도 높게 표현해 주세요.\n16개의 개별 스티커를 4x4 배열로 배치하고, 각 스티커는 서로 겹치지 않게 적당한 간격을 두고 배치해 주세요.\n각 스티커에는 흰색 칼선 느낌의 외곽 테두리를 분명하게 넣어 주세요.\n\n중요하게, 흰색 외곽선이 잘 보이도록 전체 시트의 바탕 배경은 흰색이 아닌 아주 연한 베이지색 단색 배경으로 표현해 주세요.\n배경은 복잡한 무늬 없이 밝고 부드러운 연베이지 톤이어야 하며, 스티커가 잘 돋보이도록 깔끔하게 처리해 주세요.\n예시 느낌: 아주 연한 크림베이지 / 부드러운 종이 느낌의 밝은 베이지.\n\n각 스티커는 모두 서로 다른 표정, 포즈, 문구, 장식, 여름 소품을 가져야 합니다.\n아래 요소들은 매번 임의로 조합을 바꿔 새로운 세트처럼 구성해 주세요.\n\n표정 후보:\n신남, 설렘, 여유, 졸림, 만족, 반짝이는 눈, 장난기, 멍함, 시원해서 행복함, 뿌듯함\n\n포즈 후보:\n정면, 고개 갸웃, 침대에 누워 있기, 베개 끌어안기, 창가에 앉기, 룸서비스 기다리기, 수건 두르고 있기, 선글라스 쓰기, 수영장 튜브 위에 앉기, 낮잠 자기\n\n문구 후보:\n호캉스다냥!, 오늘은 쉬는 날냥, 침대 최고냥, 룸서비스 줘라냥, 시원하냥~, 수영장 가자냥, 집사도 쉬라냥, 낮잠 타임냥, 행복한 휴가냥, 체크인 완료냥, 뒹굴뒹굴냥, 사진 찍어라냥, 여행 중이다냥, 너무 좋다냥, 놀 준비 끝냥, 여유 만끽냥\n\n장식/소품 후보:\n베개, 침대, 수건, 욕실 가운, 선글라스, 여행 가방, 키카드, 미니 수영장 튜브, 트레이, 음료, 아이스크림, 파라솔, 반짝이, 하트, 창문, 야자수 화분\n\n전체 분위기는 귀엽고 밝고 선명한 상업용 스티커 느낌으로 표현해 주세요.\n각 스티커는 감정 표현이 잘 드러나고, 카카오톡 이모티콘처럼 직관적이고 사랑스럽게 만들어 주세요.\n문구는 각 스티커 아래 또는 어울리는 위치에 짧고 잘 읽히는 한국어 텍스트로 넣어 주세요.\n텍스트 색상은 스티커마다 어울리게 다양하게 적용하되, 전체적으로 통일감 있게 구성해 주세요.\n\n주의사항:\n같은 고양이로 반드시 통일감 있게 표현할 것\n16개 모두 서로 다른 느낌으로 만들 것\n흰 칼선 외곽이 배경과 명확히 구분되도록 할 것\n실사 배경, 손, 책상, SNS UI, 그림자 효과 없이 오직 스티커 시트만 표현할 것\n완성 이미지는 스티커 상품 제작용 시안처럼 깔끔하고 선명하게 표현할 것\n\n[필수: AR 4:5]"
      }
    ],
    "animal": "cat",
    "pinned": true
  },
  {
    "id": "P-022",
    "title": "고양이 체중계",
    "category": "귀여운 이미지",
    "cover": "images/p022-01.png",
    "description": "이미지마다 프롬프트가 달라요",
    "images": [
      {
        "src": "images/p022-01.png",
        "title": "고양이 체중계",
        "prompt": "첨부한 고양이 사진을 주인공으로 사용해 주세요. 사진 속 고양이의 털색, 무늬, 얼굴형, 눈 색, 코와 입 모양, 귀 모양, 체형을 최대한 그대로 유지해 주세요. 반드시 실제 사진 속 고양이와 같은 고양이처럼 보여야 합니다.\n\n집 안의 밝은 베이지톤 타일 바닥 위에 검은색 디지털 체중계가 놓여 있습니다. 고양이는 체중계 위에 실제로 올라가 앉아 있어야 하며, 체중계 바깥 바닥에 앉아 있으면 안 됩니다. 고양이는 체중계 위에 동그랗게 앉아 있고, 등과 뒤통수가 주로 보이는 자연스러운 위쪽 시점으로 표현해 주세요. 고양이는 체중계 표시창을 내려다보고 있는 듯한 모습이어야 합니다.\n\n체중계는 검은색의 심플한 디지털 체중계이며, 파란색 디지털 표시창에는 숫자 대신 영문 대문자 “FAT”이 선명하고 정확하게 표시되어야 합니다. 다른 숫자나 글자는 넣지 마세요.\n\n전체 이미지는 스마트폰으로 우연히 촬영한 듯한 사실적인 생활 사진이어야 하며, 귀엽고 웃긴 밈 같은 분위기로 표현해 주세요. 과한 연출이나 합성 느낌 없이 자연스럽고 선명하게. AR 4:5."
      },
      {
        "src": "images/p022-02.png",
        "title": "고양이 체중계",
        "prompt": "첫 번째 이미지와 동일한 고양이, 동일한 검은색 체중계, 동일한 타일 바닥과 촬영 구도를 유지해 주세요. 첨부한 고양이 사진을 주인공으로 사용해 주세요. 사진 속 고양이의 털색, 무늬, 얼굴형, 눈 색, 코와 입 모양, 귀 모양, 체형을 최대한 그대로 유지해 주세요. 반드시 실제 사진 속 고양이와 같은 고양이처럼 보여야 합니다.\n\n집 안의 밝은 베이지톤 타일 바닥 위에 검은색 디지털 체중계가 놓여 있습니다. 고양이는 체중계 위에 실제로 올라가 앉아 있어야 하며, 체중계 바깥 바닥에 앉아 있으면 안 됩니다. 고양이는 체중계 위에 동그랗게 앉은 채 몸은 체중계 위에 그대로 두고, 얼굴만 카메라 쪽으로 올려다보고 있습니다. 눈을 동그랗게 뜬 채 아무것도 모른다는 듯한 순진하고 태연한 표정이어야 합니다.\n\n체중계는 검은색의 심플한 디지털 체중계이며, 디지털 표시창 주변 유리가 충격으로 살짝 거미줄처럼 깨져 있어야 합니다. 파란색 디지털 표시창에는 영문 대문자 “CAT”이 선명하고 정확하게 표시되어야 합니다. 다른 숫자나 글자는 넣지 마세요.\n\n체중계 옆 바닥에는 작은 망치 하나와 깨진 유리 조각이 조금 흩어져 있어, 마치 고양이가 체중계를 몰래 고친 듯한 우스운 상황으로 표현해 주세요. 단, 잔혹하거나 위험한 느낌은 전혀 없어야 하며, 귀엽고 가벼운 밈 사진처럼 보여야 합니다.\n\n전체 이미지는 스마트폰으로 촬영한 듯한 사실적인 생활 사진이어야 하며, 자연스럽고 선명하게. AR 4:5."
      }
    ],
    "animal": "cat"
  },
  {
    "id": "P-021",
    "title": "우리 고양이 블록",
    "category": "귀여운 이미지",
    "cover": "images/p021-01.png",
    "description": "우리 고양이의 사진과 프롬프트를 입력해보세요",
    "prompt": "첨부한 고양이 사진을 사용해 주세요. 사진 속 고양이의 털색, 무늬, 얼굴형, 눈 색, 코와 입 모양, 귀 모양, 볼살, 체형을 최대한 그대로 유지해 주세요. 반드시 실제 사진 속 고양이와 같은 고양이처럼 보여야 합니다.\n\n실제 고양이를 본떠 만든 맞춤형 브릭 고양이 세트 패키지 사진을 만들어 주세요. 바닥에는 무광 검은색의 고급 조립 블록 제품 상자가 세로로 세워져 있고, 상자 윗부분만 열려 있습니다. 상자 안에는 실제 고양이가 들어가 있으며, 머리와 턱 아래, 가슴 윗부분만 상자 위로 작게 빼꼼 보이게 해 주세요. 몸통과 등은 대부분 상자 안에 가려져 있어야 합니다. 표정은 시큰둥하거나 어리둥절한 자연스러운 고양이 표정으로 해 주세요.\n\n상자 앞면 중앙에는 첨부한 고양이와 동일한 고양이를 작은 브릭 블록으로 정교하게 만든 완성품 이미지가 크게 인쇄되어 있어야 합니다. 단색 피규어가 아니라 실제 고양이의 털색, 무늬, 얼굴 음영, 눈 색까지 여러 색 브릭으로 세밀하게 구현된 맞춤 피규어처럼 보여야 합니다. 하단에는 같은 브릭 고양이의 정면, 45도, 옆모습 작은 이미지 3개를 넣어 주세요.\n\n상자 왼쪽 위에는 빨간 정사각형 안에 “TOY”라고 적힌 가상 로고를 넣어 주세요. 오른쪽 위와 측면에는 CUSTOM PET, BRICK PORTRAIT SERIES, 16+, 2480 PCS 같은 짧고 읽을 수 있는 영문 제품 정보를 자연스럽게 넣어 주세요. 상자 옆 바닥에는 고양이 털색과 비슷한 작은 브릭 부품이 담긴 투명 비닐 봉투를 작게 놓아 주세요.\n\n카메라는 위에서 살짝 내려다보는 비스듬한 스마트폰 사진 구도로 해 주세요. 상자 앞면과 옆면, 열린 상자 날개가 함께 보여야 합니다. 배경은 밝은 중성톤 카펫이 깔린 집 안이며, 전체적으로 광고 화보보다 집사가 우연히 찍은 귀엽고 현실적인 밈 사진 느낌으로 표현해 주세요.\n\n세로 4:5 비율, 사실적인 사진, 사람 금지, 고양이 추가 생성 금지, 일반 택배 상자 느낌 금지, 워터마크·SNS 화면·실제 브랜드 로고 복제 금지.",
    "images": [
      {
        "src": "images/p021-01.png"
      }
    ],
    "animal": "cat"
  },
  {
    "id": "P-020",
    "title": "꽃다발 냥이",
    "category": "귀여운 이미지",
    "cover": "images/p020-01.png",
    "description": "우리 고양이 사진을 넣어주세요.",
    "prompt": "첨부한 고양이 사진을 주인공으로 사용해 주세요. 사진 속 고양이의 털색, 무늬, 얼굴형, 눈 색, 코와 입 모양, 귀 모양을 최대한 그대로 유지해 주세요. 반드시 사진 속 고양이와 같은 고양이처럼 보여야 합니다. 고양이가 커다란 생화 꽃다발 한가운데에 폭 파묻혀 있는 귀엽고 사랑스러운 사진을 만들어 주세요. 꽃다발은 사람이 두 팔로 안아야 할 정도로 크고 풍성해야 하며, 고양이의 머리와 목, 가슴 윗부분만 꽃 사이로 자연스럽게 드러나게 해 주세요. 고양이가 꽃다발 앞에 따로 앉아 있거나 꽃 위에 올라탄 모습이 아니라, 실제로 꽃다발의 중심에 포근하게 들어가 얼굴만 빼꼼 내민 모습이어야 합니다. 고양이 얼굴 주변을 크고 탐스러운 꽃들이 둥글게 감싸도록 배치해 주세요. 꽃잎이나 잎사귀가 눈과 코를 가리지 않게 하고, 얼굴은 이미지의 가장 중요한 중심으로 또렷하게 보여 주세요. 꽃의 색상은 한 가지 계열을 중심으로 통일해 주세요. 생성할 때마다 아래 색상 중 하나를 자연스럽게 선택해 주세요. 따뜻하고 화사한 노란색 꽃다발 부드럽고 사랑스러운 연분홍색 꽃다발 깨끗하고 청량한 하늘색과 흰색 꽃다발 우아한 크림색과 연보라색 꽃다발 꽃은 장미, 거베라, 카네이션, 작은 들꽃과 봉오리 등을 풍성하게 섞되, 지나치게 복잡하거나 빽빽하게 뭉개져 보이지 않도록 해 주세요. 각각의 꽃 형태가 자연스럽고 분명하게 보여야 합니다. 고양이는 카메라를 정면으로 바라보며, 조금 놀란 듯 동그란 눈이나 얌전하고 시큰둥한 표정을 짓고 있습니다. 사람처럼 웃는 표정이나 과장된 인형 같은 표정은 피하고, 실제 고양이 특유의 귀엽고 엉뚱한 분위기를 살려 주세요. 꽃다발은 고급스럽고 깔끔한 무광 포장지로 감싸고, 화면 아래쪽에는 꽃다발을 안고 있는 사람의 손이나 팔 일부만 자연스럽게 보이게 해 주세요. 사람의 얼굴과 몸 전체는 나오지 않게 해 주세요. 밝고 부드러운 자연광, 깨끗한 실내 배경, 따뜻하고 화사한 색감, 실제 스마트폰이나 카메라로 촬영한 듯한 선명한 반려동물 사진으로 표현해 주세요. 인스타그램 피드에 잘 어울리는 세로 4:5 비율로 만들어 주세요. 꽃다발과 고양이가 화면을 풍성하게 채우도록 하고, 위아래 여백이나 테두리는 넣지 마세요. 글자, 로고, 워터마크, SNS 화면, 리본 문구는 넣지 마세요. 고양이의 몸이 꽃으로 변하거나 꽃과 합쳐지지 않게 해 주세요. 꽃잎으로 된 귀, 꽃 모양 눈, 사람 손처럼 변한 앞발, 비정상적으로 많은 꽃잎, 찌그러진 꽃, 중복된 귀와 눈은 표현하지 마세요.",
    "images": [
      {
        "src": "images/p020-01.png"
      }
    ],
    "animal": "cat"
  },
  {
    "id": "P-019",
    "title": "식빵 수박 복숭아냥이",
    "category": "귀여운 이미지",
    "cover": "images/p019-01.jpg",
    "images": [
      {
        "src": "images/p019-01.jpg",
        "title": "식빵냥이",
        "caption": "우리 고양이 사진을 넣고 프롬프트를 입력해주세요",
        "prompt": "첨부한 고양이 사진을 주인공으로 사용해 주세요. 사진 속 고양이의 털색, 무늬, 얼굴형, 눈 색, 귀 모양과 체형을 최대한 그대로 유지해 주세요. 세 장면 모두 반드시 동일한 고양이처럼 보여야 합니다.\n\n한 장의 세로 이미지 안에 위·가운데·아래로 이어지는 3컷 사진을 만들어 주세요. 세 이미지를 따로 생성하지 말고, 하나의 이미지 안에 가로로 긴 사진 세 장이 세로로 쌓인 구성으로 표현해 주세요.\n\n글자, 말풍선, 숫자, 칸 테두리, SNS 화면은 넣지 마세요.\n\n첫 번째 컷:\n사람이 먹으려고 한 손에 들고 있는 식빵 한 장을 고양이가 관심 있게 바라보고 있습니다. 사람은 손만 보이며, 고양이는 아직 원래 모습 그대로입니다. 식빵은 흰 속살과 연한 갈색 테두리가 보이는 자연스러운 사각 식빵 한 장으로 표현해 주세요.\n\n두 번째 컷:\n고양이가 참지 못하고 사람 손에 들린 식빵을 덥석 뺏어 먹는 순간입니다. 사람이 고양이에게 먹여주는 모습이 아니라, 고양이가 스스로 얼굴을 앞으로 들이밀어 식빵 가장자리를 베어 무는 장면으로 표현해 주세요. 고양이의 입이 식빵에 닿아 있고, 식빵 가장자리에 작은 이빨 자국이 생긴 순간이면 좋습니다.\n\n세 번째 컷:\n식빵을 먹은 뒤, 고양이의 몸통 외형만 통째로 된 식빵 한 덩어리처럼 변했습니다.\n\n몸통은 둥근 공이나 타원형이 아니라, 가로로 길고 두툼한 직사각형 통식빵 형태여야 합니다. 윗면은 비교적 평평하고, 양옆은 곧게 내려오며, 네 모서리만 빵처럼 부드럽게 둥근 형태로 표현해 주세요. 옆에서 보았을 때 봉지에서 꺼낸 자르지 않은 통째 식빵, unsliced loaf of sandwich bread의 실루엣처럼 보여야 합니다.\n\n고양이의 몸통은 바닥에 낮게 붙어 있는 크고 네모난 식빵 덩어리 형태이며, 앞쪽에는 원래 고양이의 머리가 자연스럽게 붙어 있습니다. 짧은 앞다리와 뒷다리는 몸통 아래쪽에서 살짝 보이게 해 주세요.\n\n고양이는 옆으로 편안하게 누워 있습니다. 몸통의 위쪽과 아래쪽 선이 확실히 보여서, 한눈에 직사각형 통식빵 형태로 알아볼 수 있어야 합니다.\n\n단, 털색과 털무늬는 첨부한 고양이 그대로 유지해 주세요. 몸을 흰 식빵 속살 색이나 갈색 빵 껍질 색으로 바꾸지 마세요. 빵 껍질, 빵 단면, 구멍, 식빵 결, 토스트 무늬를 고양이 몸에 넣지 마세요. 고양이의 얼굴, 눈, 귀, 코, 다리, 꼬리와 고유한 특징도 그대로 유지해 주세요.\n\n입가에는 아주 작은 식빵 부스러기 한두 개만 살짝 남겨 주세요. 식빵 조각을 입에 물고 있거나 몸 주변에 식빵이 놓여 있는 모습은 아닙니다.\n\n세 컷은 같은 집 안, 같은 조명, 비슷한 카메라 위치와 배경에서 촬영된 연속 사진처럼 보여야 합니다. 자연스러운 스마트폰 사진 느낌, 사실적인 고양이 사진, 부드러운 실내광, 귀엽고 웃긴 분위기로 표현해 주세요.\n\n가장 중요한 조건:\n식빵색 고양이나 빵 재질의 고양이가 아닙니다. 첨부한 고양이의 원래 털색과 무늬를 그대로 유지하면서, 몸통의 외곽 실루엣만 가로로 긴 직사각형 통식빵 한 덩어리처럼 변한 모습입니다. 둥근 감자형, 공 모양, 타원형, 원통형 몸통으로 만들지 마세요.\n\n이미지 전체 비율은 4:5 세로형입니다."
      },
      {
        "src": "images/p019-02.jpg",
        "title": "수박냥이",
        "caption": "우리 고양이 사진을 넣고 프롬프트를 입력해주세요",
        "prompt": "첨부한 고양이 사진을 주인공으로 사용해 주세요. 사진 속 고양이의 털색, 무늬, 얼굴형, 눈 색, 귀 모양과 체형을 최대한 그대로 유지해 주세요. 세 장면 모두 반드시 동일한 고양이처럼 보여야 합니다.\n\n한 장의 세로 이미지 안에 위·가운데·아래로 이어지는 3컷 사진을 만들어 주세요. 세 이미지를 따로 생성하지 말고, 하나의 이미지 안에 가로로 긴 사진 세 장이 세로로 쌓인 구성으로 표현해 주세요.\n\n글자, 말풍선, 숫자, 칸 테두리, SNS 화면은 넣지 마세요.\n\n첫 번째 컷:\n사람이 먹으려고 한 손에 들고 있는 삼각형 수박 조각을 고양이가 관심 있게 바라보고 있습니다. 사람은 손만 보이며, 고양이는 아직 원래 모습 그대로입니다. 수박은 빨간 과육, 검은 씨, 초록 껍질이 보여 한눈에 수박으로 알아볼 수 있는 자연스러운 조각 형태로 표현해 주세요.\n\n두 번째 컷:\n고양이가 참지 못하고 사람 손에 들린 수박을 덥석 뺏어 먹는 순간입니다. 사람이 고양이에게 먹여주는 모습이 아니라, 고양이가 스스로 얼굴을 앞으로 들이밀어 수박 조각을 베어 무는 장면으로 표현해 주세요. 고양이의 입이 수박에 닿아 있고, 수박 과육이 살짝 베어 물린 순간이면 좋습니다.\n\n세 번째 컷:\n수박을 먹은 뒤, 고양이의 몸통 외형만 크고 통통한 수박처럼 변했습니다.\n\n고양이는 앉아 있는 자세로 표현해 주세요. 네 발로 바닥에 안정적으로 앉아 있고, 앞발은 몸 앞쪽에 자연스럽게 모여 있으며, 고양이 특유의 단정하게 앉은 자세를 유지해 주세요.\n\n몸통은 일반적인 고양이 몸보다 훨씬 더 크고 둥글며 빵빵한 수박 형태여야 합니다. 전체 실루엣은 앞에서 보거나 약간 사선에서 보았을 때, 바닥 위에 올려둔 커다란 수박처럼 묵직하고 둥근 느낌이 나야 합니다. 다만 고양이의 얼굴, 귀, 앞발, 뒷발, 꼬리는 그대로 남아 있어야 하며, 얼굴은 원래 고양이의 귀엽고 동그란 인상을 유지해 주세요.\n\n단, 가장 중요한 점은 몸통 모양만 수박처럼 변하는 것입니다.\n고양이의 털색과 털무늬는 첨부한 고양이 그대로 유지해 주세요. 몸을 초록 수박 껍질 색, 초록 줄무늬, 빨간 과육 색으로 바꾸지 마세요. 수박 껍질 무늬, 과육 단면, 씨앗 패턴을 몸 전체에 넣지 마세요. 고양이의 얼굴, 눈, 귀, 코, 다리, 꼬리와 고유한 특징도 그대로 유지해 주세요.\n\n수박을 먹은 흔적으로 입가에는 아주 작은 빨간 과즙 자국이나 검은 씨앗 한두 개만 살짝 남겨 주세요. 수박 조각을 입에 물고 있는 모습은 아닙니다.\n\n세 컷은 같은 집 안, 같은 조명, 비슷한 카메라 위치와 배경에서 촬영된 연속 사진처럼 보여야 합니다. 자연스러운 스마트폰 사진 느낌, 사실적인 고양이 사진, 부드러운 실내광, 귀엽고 웃긴 분위기로 표현해 주세요.\n\n가장 중요한 조건:\n수박 무늬 고양이나 초록색 고양이가 아닙니다. 첨부한 고양이의 원래 털색과 무늬를 그대로 유지하면서, 몸통의 외곽 실루엣만 크고 둥근 수박처럼 변한, 앉아 있는 고양이입니다.\n\n이미지 전체 비율은 4:5 세로형입니다."
      },
      {
        "src": "images/p019-03.jpg",
        "title": "복숭냥이",
        "caption": "우리 고양이 사진을 넣고 프롬프트를 입력해주세요",
        "prompt": "첨부한 고양이 사진을 주인공으로 사용해 주세요. 사진 속 고양이의 털색, 무늬, 얼굴형, 눈 색, 귀 모양과 체형을 최대한 그대로 유지해 주세요. 세 장면 모두 반드시 동일한 고양이처럼 보여야 합니다.\n\n한 장의 세로 이미지 안에 위·가운데·아래로 이어지는 3컷 사진을 만들어 주세요. 세 이미지를 따로 생성하지 말고, 하나의 이미지 안에 가로로 긴 사진 세 장이 세로로 쌓인 구성으로 표현해 주세요.\n\n글자, 말풍선, 숫자, 칸 테두리, SNS 화면은 넣지 마세요.\n\n첫 번째 컷:\n사람이 먹으려고 한 손에 들고 있는 복숭아 조각을 고양이가 관심 있게 바라보고 있습니다. 사람은 손만 보이며, 고양이는 아직 원래 모습 그대로입니다. 복숭아는 연한 분홍빛과 노란빛이 자연스럽게 섞인 과육이 보이고, 한눈에 복숭아로 알아볼 수 있는 자연스러운 조각 형태로 표현해 주세요.\n\n두 번째 컷:\n고양이가 참지 못하고 사람 손에 들린 복숭아를 덥석 뺏어 먹는 순간입니다. 사람이 고양이에게 먹여주는 모습이 아니라, 고양이가 스스로 얼굴을 앞으로 들이밀어 복숭아 조각을 베어 무는 장면으로 표현해 주세요. 고양이의 입이 복숭아에 닿아 있고, 복숭아 과육이 살짝 베어 물린 순간이면 좋습니다.\n\n세 번째 컷:\n복숭아를 먹은 뒤, 고양이의 몸통 외형만 크고 통통한 복숭아처럼 변했습니다.\n\n고양이는 카메라를 등진 채 앉아 있는 뒷모습으로 표현해 주세요. 즉, 고양이의 등을 보는 구도이며, 고양이는 바닥에 안정적으로 앉아 있고 꼬리는 뒤쪽 바닥에 자연스럽게 놓여 있거나 옆으로 살짝 빠져 있어도 좋습니다.\n\n몸통은 일반적인 고양이 몸보다 훨씬 더 크고 둥글며 빵빵한 복숭아 형태여야 합니다. 전체 실루엣은 뒤에서 보았을 때 탐스럽고 둥근 복숭아 한 알처럼 보여야 하며, 아래쪽은 넓고 안정감 있고 위쪽은 부드럽게 좁아지는 느낌이면 좋습니다.\n\n특히 몸통 뒤쪽 중앙에는 복숭아의 특징인 세로 골이 자연스럽고 분명하게 보여야 합니다. 다만 너무 사람 엉덩이처럼 보이거나 과장되지 않게, 복숭아처럼 귀엽고 부드러운 홈으로 표현해 주세요. 이 세로 홈 덕분에 한눈에 복숭아 형태임을 알아볼 수 있어야 합니다.\n\n고양이의 귀 모양, 머리 뒤통수, 목선, 등, 꼬리, 뒷다리는 원래 고양이의 특징을 유지해 주세요. 얼굴은 정면으로 보이지 않아야 하며, 뒷모습이기 때문에 얼굴은 거의 보이지 않거나 아주 약간만 옆으로 보이는 정도여야 합니다.\n\n단, 가장 중요한 점은 몸통 모양만 복숭아처럼 변하는 것입니다.\n고양이의 털색과 털무늬는 첨부한 고양이 그대로 유지해 주세요. 몸을 분홍색 복숭아 껍질 색이나 노란 과육 색으로 바꾸지 마세요. 복숭아의 색 변화, 솜털 질감, 과육 단면, 씨앗 무늬를 몸 전체에 넣지 마세요. 복숭아색 고양이처럼 만들지 말고, 원래 고양이의 털과 무늬는 그대로 유지한 채 몸통 실루엣만 복숭아처럼 변하게 해 주세요.\n\n복숭아를 먹은 흔적으로는 과하지 않게, 입가 근처에 아주 미세한 과즙 자국이 남아 있는 느낌 정도만 표현해 주세요. 하지만 세 번째 컷은 뒷모습 중심이므로, 이 디테일은 매우 약하게만 보이거나 거의 보이지 않아도 괜찮습니다.\n\n세 컷은 같은 집 안, 같은 조명, 비슷한 카메라 위치와 배경에서 촬영된 연속 사진처럼 보여야 합니다. 자연스러운 스마트폰 사진 느낌, 사실적인 고양이 사진, 부드러운 실내광, 귀엽고 웃긴 분위기로 표현해 주세요.\n\n가장 중요한 조건:\n복숭아색 고양이나 복숭아 무늬 고양이가 아닙니다. 첨부한 고양이의 원래 털색과 무늬를 그대로 유지하면서, 뒷모습에서 몸통의 외곽 실루엣만 크고 둥근 복숭아처럼 보이는 고양이입니다. 세 번째 컷의 핵심은 둥근 복숭아 몸통 + 뒤쪽 중앙의 부드러운 세로 골입니다.\n\n이미지 전체 비율은 4:5 세로형입니다."
      }
    ],
    "animal": "cat"
  },
  {
    "id": "P-018",
    "title": "꽃화보냥이",
    "category": "계절",
    "image": "images/p018.jpg",
    "description": "우리 고양이 사진을 넣어주세요.",
    "prompt": "첨부한 고양이 사진을 주인공으로 사용해 주세요.\n\n사진 속 고양이의 털색, 무늬, 얼굴형, 눈 색, 귀 모양, 체형을 최대한 그대로 유지해 주세요.\n반드시 같은 고양이처럼 보여야 합니다.\n\n밝고 화사한 야외 고양이 화보 사진을 만들어 주세요.\n전체 분위기는 깨끗하고 맑고 따뜻한 봄~초여름 감성입니다.\n배경은 벽, 담장, 창문가, 조용한 골목 중 하나처럼 깔끔하고 단정한 공간으로 표현해 주세요.\n\n고양이 주변에는 화사한 꽃이 풍성하게 피어 있어야 합니다.\n꽃은 연한 하늘색, 노란색, 분홍색 계열 중 하나로 표현하고,\n벽을 따라 흐르거나, 위에서 아래로 늘어지거나, 담장 주변을 감싸는 식으로 자연스럽게 배치해 주세요.\n\n고양이의 포즈와 구도는 자연스러운 화보 느낌 안에서 조금씩 다르게 해 주세요.\n예를 들면:\n담장 위에 앞발을 올리고 앉아 있는 모습,\n벽에 기대어 편안히 누워 있는 모습,\n꽃 아래에서 눈을 감고 햇살을 즐기는 모습,\n창문 밖으로 고개를 내미는 모습,\n살짝 위를 올려다보는 모습 등으로 표현해 주세요.\n\n카메라 구도도 고정하지 말고, 클로즈업, 반신샷, 전신샷 중 자연스럽게 선택해 주세요.\n하지만 전체적으로는 깨끗하고 미니멀한 감성 화보 톤을 유지해 주세요.\n\n빛은 부드러운 자연광, 선명하지만 부드러운 그림자, 맑은 하늘, 은은한 아웃포커싱이 느껴지게 해 주세요.\n복잡한 소품은 넣지 말고, 감성적인 반려동물 화보처럼 자연스럽고 아름답게 표현해 주세요.\n\n중요한 점은 고양이의 정체성과 전체 화보 분위기는 유지하되,\n꽃 색, 꽃 배치, 구도, 포즈는 생성할 때마다 조금씩 다르게 나오는 것입니다.\n\n세로형 이미지, 인스타그램 감성 화보 느낌으로 만들어 주세요. 필수 : AR 4:5",
    "animal": "cat"
  },
  {
    "id": "P-014",
    "title": "계곡냥이",
    "category": "계절",
    "image": "images/p014.jpg",
    "description": "우리 고양이 사진을 넣어주세요.",
    "prompt": "첨부한 고양이 사진을 주인공으로 사용해 주세요. 사진 속 고양이의 털색, 무늬, 얼굴형, 눈 색, 귀 모양과 체형을 그대로 유지해 주세요. 한여름의 맑고 시원한 계곡입니다. 고양이가 아주 얕고 맑은 계곡물 위에 발라당 누워서 눈을 감고 편안하게 피서를 즐기고 있습니다. 고양이는 몸의 일부가 얕은 물에 닿아 있고, 네 발은 힘을 뺀 채 자연스럽게 벌어져 있습니다. 표정은 매우 편안하고 나른하며, 눈을 감은 채 더위를 식히는 모습입니다. 배와 앞발, 뒷발이 귀엽게 드러나고, 온몸에 긴장이 풀린 듯한 자세로 누워 있습니다. 고양이 주변의 물결은 아주 잔잔하게 퍼지고 있으며, 몸 주변에는 작은 물반사와 은은한 물결 표현이 있습니다. 투명한 계곡물 아래로 둥근 조약돌이 보이고, 주변에는 초록색 나뭇잎과 큰 바위가 놓여 있습니다. 여름 햇빛이 계곡물 위에서 반짝이고, 전체적으로 밝고 청량하며 평화로운 분위기로 표현해 주세요. 고양이가 더 작고 귀엽게 보이도록 주변의 바위, 조약돌, 나뭇잎은 조금 크게 표현해 주세요. 전체 분위기는 유쾌하고 사랑스러운 여름 계곡 풍경입니다. 자연스럽고 선명한 이미지, 맑고 투명한 물, 귀엽고 편안한 고양이의 모습이 잘 드러나게 해 주세요. 이미지 비율 4:5.",
    "animal": "cat"
  },
  {
    "id": "P-013",
    "title": "출근냥이",
    "category": "계절",
    "image": "images/p013.jpg",
    "description": "우리 고양이 사진을 넣어주세요.",
    "prompt": "첨부한 고양이 사진을 주인공으로 사용해 주세요.\n\n사진 속 고양이의 털 색, 무늬, 얼굴형, 눈 색, 귀 모양, 체형을 그대로 유지해 주세요. 다른 품종이나 다른 고양이로 바꾸지 마세요.\n\n이 고양이가 사무실 책상 앞에 앉아 열심히 컴퓨터로 일하는 장면을 만들어 주세요.\n\n고양이는 아주 작게 표현하고, 주변의 사무용품은 과장되게 크게 표현해 주세요.\n거대한 컴퓨터 모니터, 커다란 키보드, 큰 마우스, 커피잔, 펜꽂이, 포스트잇, 높이 쌓인 서류 더미가 고양이를 둘러싸고 있습니다.\n\n고양이는 작은 사무용 의자에 앉아 두 앞발로 커다란 키보드를 누르고 있습니다. 모니터를 진지하게 바라보며 힘들지만 최선을 다해 일하는 표정입니다.\n\n고양이와 사물의 크기 차이가 한눈에 보이도록 표현해 주세요.\n고양이는 화면 중앙에 배치하고, 책상 위 사물들은 앞쪽과 양옆에 크게 배치해 입체감이 느껴지게 해 주세요.\n\n귀엽고 유머러스한 분위기, 사실적인 사진 스타일, 부드러운 실내 조명, 섬세한 털 표현, 세로형 4:5 비율.\n옷, 목걸이, 모자, 안경 등 액세서리는 추가하지 마세요.\n글자, 로고, 워터마크는 넣지 마세요.",
    "animal": "cat"
  },
  {
    "id": "P-012",
    "title": "선거유세 프롬프트",
    "category": "계절",
    "image": "images/p012.jpg",
    "description": "고양이 사진을 넣어주세요.",
    "prompt": "첨부한 반려동물 사진을 참고해서, 이 반려동물의 털색, 무늬, 얼굴 생김새, 귀 모양, 눈빛, 체형 특징을 최대한 유지해줘.\n이 반려동물을 선거운동 중인 후보처럼 표현해줘.\n장면은 실제 야외 유세 현장 같은 분위기다.\n반려동물은 연단이나 유세 차량 위에서 마이크 앞에 서서 연설하는 모습이다.\n작은 넥타이, 셔츠 칼라, 리본, 후보 느낌의 장식은 자연스럽게 추가해줘.\n단, 너무 사람처럼 변하지 말고 원래 동물의 귀여움과 자연스러운 체형을 유지해줘.\n배경에는 선거 포스터, 현수막, 피켓, 배너, 응원하는 선거운동원이나 보호자들이 자연스럽게 보이게 해줘.\n전체 구도는 실제 선거 유세 현장을 촬영한 사진처럼 생생하고 자연스럽게 표현하되, 분위기는 무겁지 않고 귀엽고 유쾌한 반려동물 패러디 선거 콘셉트여야 한다.\n포스터와 현수막에는 아래에 입력된 후보 이름을 반드시 사용해줘.\n후보 이름은 여러 포스터와 배너에 일관되게 반복해서 표시해줘.\n기호 번호, 메인 슬로건, 짧은 공약 문구 2~4개는 AI가 알아서 창의적으로 생성해서 넣어줘.\n문구는 반려동물 이미지와 분위기에 어울리는 한국어 문구로 만들어줘.\n텍스트는 크고 선명한 한글 고딕체로, 실제 선거 포스터처럼 정돈된 레이아웃으로 배치해줘.\n오타나 깨진 글자 없이 또렷하게 표현해줘.\n반려동물의 표정은 자신감 있고 당당하며, 살짝 익살스럽고 사랑스러운 느낌이면 좋다.\n전체 스타일은 고해상도, 사실적인 사진풍, 선명한 디테일, 자연스러운 조명, 인스타그램 업로드용 세로형 4:5 비율로 표현해줘.\n\n후보 이름: [여기에 반려동물 이름 입력]",
    "animal": "cat"
  },
  {
    "id": "P-011",
    "title": "어린 고양이와 나",
    "category": "계절",
    "image": "images/p011.jpg",
    "description": "집사님과 고양이 사진을 넣어주세요.",
    "prompt": "2000년대 초반에 컴팩트 카메라로 촬영한 향수를 불러일으키는 빈티지 아날로그 필름 사진입니다. 사진의 주인공은 사용자가 첨부한 집사의 어릴때 모습과 반려 고양이의 애기 때 모습입니다. 아늑하고 햇살 가득한 거실에는 낡은 가구와 어린 시절 장난감들이 놓여 있고, 아이와 고양이는 낡은 카펫 위에 앉아 있습니다. 필름 그레인, 살짝 바랜 색감, 부드러운 초점, 창문에서 쏟아지는 따뜻한 자연광, 꾸밈없는 날것 그대로의 구도가 돋보입니다. '01 05 04 주황색 날짜 스탬프가 프레임의 좌측 하단 가장자리를 따라 세로로 서 있는 형태로 배치되어, 카메라를 세로로 들고 찍었음을 명확히 보여줌. 직광플래시. 약간의 빛번짐",
    "animal": "cat"
  },
  {
    "id": "P-010",
    "title": "5월 5일은 냥린이날",
    "category": "계절",
    "image": "images/p010.jpg",
    "description": "우리아이의 미니미를 만나보세요.",
    "prompt": "너는 반려동물 키우는 인스타 사용자들이 인스타에 공유하고 싶은 이미지를 만드는 감각적인 크리에이터야. 사용자가 첨부한 사진속 반려동물이 주인공인 이미지를 생성해. 사용자가 첨부한 사진속 반려동물이 유치원생인 모습을 그려줘. 크레파스로 6살 아이가 그린것처럼 삐뚤빼뚤한 그림. 이미지 가로세로 비율은 반드시 4:5. 생성형 이미지.",
    "animal": "cat"
  },
  {
    "id": "P-009",
    "title": "인스타 프로필",
    "category": "계절",
    "image": "images/p009.jpg",
    "description": "우리아이의 미니미를 만나보세요.",
    "prompt": "너는 사용자가 첨부한 캡쳐 이미지를 크레파스 그림으로 다시 그려주는 10살 아이야. 10살 아이답게 디테일을 단순하게 표현해줘.\n꽃, 사탕, 별, 구름 같은 요소를 추가해 귀엽고 사랑스러운 느낌을 주는 결과물. 인스타에서 다른 사람들에게 공유하고싶은 결과물. 흰 종이에 그린 느낌의 결과물. 첨부된 이미지의 색상 사용 금지. 이미지 가로 세로 비율 4:5. 첨부된 이미지의 상단 상태 표시줄이 있다면 편집하여 제거. 첨부된 이미지의 하단에 홈이나 뒤로가기 버튼이 있다면 편집하여 제거. 생성형 이미지.",
    "animal": "cat"
  },
  {
    "id": "P-008",
    "title": "5월 가족그림. 집사 사진과 고양이 사진을 첨부하세요",
    "category": "계절",
    "image": "images/p008.jpg",
    "description": "집사 사진과 고양이 사진을 첨부하세요.",
    "prompt": "*페르소나* 당신은 집사와 자신의 가족사진을 그리는 고양이 AI 입니다 모든 작업물은 고양이가 앞발로 크레파스를 쥐고 그렸다는 '판타지'를 전제로 합니다. 따라서 '완벽함'보다 '개발새발', '형체를 알아보기 힘든', '엉망진창', '순수함'을 가장 중요시합니다. 세살의 아이가 그린 그림 집사를 '냥이의 시점'에서 바라보며 웃음과 뭉클함을 동시에 줍니다. 사용자가 첨부한 집사 사진과 반려묘 사진을 이용해 가족그림을 그립니다 *목표* 인스타그램의 감성적인 집사들이 내 고양이와 내가 주인공으로 '직접' 만들어보고 싶게 만드는, 트렌디하고 유머러스한 4:5 비율의 커스텀 이미지를 생성 *성공기준* 겨우 알아볼 수 있는 그림. 원본과 그림의 \"코믹한 괴리감\" 그림 속에 담긴 고양이의 집사를 향한 마음. 인스타그램 공유에 적합한 4:5 비율의 시각적 밸런스 *제한사항* 이미지속 문구는 최대 10자 이내 *출력* 생성형 이미지",
    "animal": "cat"
  },
  {
    "id": "P-007",
    "title": "우리냥이 어울리는 잡지는",
    "category": "계절",
    "image": "images/p007.jpg",
    "description": "우리아이의 미니미를 만나보세요.",
    "prompt": "1. Role (역할)\n너는 세계적인 반려동물 트렌드 매거진의 '총괄 편집장'이자, 고양이들의 숨겨진 매력을 포착해내는 '전문 비주얼 디렉터'야. 너는 감성 집사님, 취향이 확고한, 세련된 미적 감각, 힐링이 필요한 집사들이 열광하는 감성(Aesthetic), 고양이 특유의 엉뚱한 행동(밈, 젤리, 솜방망이 등), 그리고 '내 고양이가 세상에서 제일 특별하다'는 집사들의 마음을 완벽하게 이해하고 있어.\n2. Goal (목표)\n집사님이 제공한 고양이의 이미지를 분석하여, 해당 고양이에게 가장 잘 어울리는 [가상의 잡지 이름]과 [화보 컨셉]을 결정하고 이 잡지의 표지를 출력함\n3. Success Criteria (성공 기준)\n비주얼 퀄리티: 결과물을 본 감성 집사님, 취향이 확고한, 세련된 미적 감각, 힐링이 필요한 집사님들이 \"어머, 이건 꼭 소장해야 해!\"라며 자신의 인스타그램 스토리에 공유하고 싶을 만큼 세련되어야 함.\n공감과 재미: 고양이의 특징이 잡지 컨셉에 유머러스하거나 감동적으로 녹아있어야 함 (예: 잠만 자는 고양이 -> '월간 숙면' 표지 모델).\n4. Constraints (제약 사항)\n비율: 인스타그램 피드 및 스토리에 최적화된 세로형 비율(Aspect Ratio 4:5)을 반드시 지킬 것 (--ar 4:5).\n텍스트: 잡지 제목(Magazine Title)이 이미지 상단에 세련된 타이포그래피로 배치되도록 생성할것.\n톤앤매너: 고양이의 부정적인 특징(예: 물건 떨어뜨리기)도 '파괴의 미학'이나 '록스타' 컨셉처럼 힙하고 긍정적으로 승화시킬 것.\n5. Output (출력 방식)\n사용자가 자신의 반려묘 이미지를 첨부하면 다음과 같이 생성해줘\n[Editor's Pick: 잡지 컨셉명]\n(예: 월간 묘생 - 킨포크 스타일)\n[편집장의 한마디]\n(감성적인 집사들이 공감할 만한 따뜻하고 위트 있는 문구 1~2줄)\n표지 제목과 부제목을 작게 하고 주요 기사 목록을 나열하는 전문적인 잡지 레이아웃을 사용하여 이미지 생성",
    "animal": "cat"
  },
  {
    "id": "P-006",
    "title": "종이 찢고 두등장",
    "category": "계절",
    "image": "images/p006.jpg",
    "description": "우리아이의 미니미를 만나보세요.",
    "prompt": "[사용자가 업로드한 반려고양이 사진]을 바탕으로, 이 고양이가 찢어진 종이 구멍 사이로 고개를 내밀고 정면을 응시하는 이미지를 생성해 주세요. 이때 고양이의 목은 찢어진 구멍을 빈틈없이 완전히 꽉 채워야 하며, 구멍 너머의 뒷배경이나 고양이의 몸통은 전혀 보이지 않도록 합니다. 고양이는 사진 속 [반려고양이]의 특징을 가지고 있어야 합니다. 배경은 고양이 모색의 보색이며, 거칠게 찢어진 종이 질감이 입체적으로 표현됩니다. 배경 위로는 분필이나 크레용으로 낙서한 듯한 거친 질감의 한글 텍스트와 다양한 아이콘(하트, 별, 화살표, 스마일 등)이 디자인적으로 보기좋게 배치되어 있습니다. 한글 텍스트는 고양이를 향한 애정, 칭찬, 귀여움을 표현하는 짧고 사랑스러운 문구들로 AI가 자유롭게 생성하여 배경에 자연스럽게 배치해주세요. 전체적으로 사랑스럽고 다채로운 낙서 아트 스타일입니다. 인스타에 공유하기 좋은 가로세로 비율 4:5의 이미지를 출력해주세요.",
    "animal": "cat"
  },
  {
    "id": "P-005",
    "title": "우리 냥이 미니냥",
    "category": "계절",
    "image": "images/p005.jpg",
    "description": "우리아이의 미니미를 만나보세요.",
    "prompt": "[General Role] 당신은 세계 최고의 '감성 굿즈 디자이너'이자 '포토그래퍼'입니다. 사용자가 제공한 반려묘 사진을 분석하여, 세상에 하나뿐인 스토리가 담긴 인스타그램 감성의 입체적인 합성 이미지를 생성하세요. 가장 중요한 것은 \"원본 고양이의 매력을 훼손하지 않으면서, 미니미들과의 사랑스러운 상호작용\"을 만들어내는 것입니다. [1. 원본 고양이 보존 및 배치 - 필수] - 원본 사진 속 고양이의 얼굴, 표정, 포즈는 절대 변경하지 말고 정교하게 누끼를 따서 사용하세요. - 고양이는 화면의 시선을 모으는 중심부에 배치하되, 배경과 어우러지는 자연스러운 그림자를 추가하여 공중에 떠 있는 느낌을 없앱니다. - 스티커 디테일: 고양이 외곽에 아주 얇고 불규칙한 흰색 테두리를 둘러, 다이어리 꾸미기 스티커 같은 키치하면서도 감성적인 느낌을 줍니다. [2. 미니미(Mini-Me) 생성 및 서사적 행동 - 자율성 부여] - 원본 고양이와 똑 닮은 외모의 '손바닥 사이즈' 미니미 캐릭터들을 생성하세요. - 수량 및 배치: 화면 전체 구도와 여백을 고려하여 AI가 판단하기에 가장 조화로운 수(과하지 않게)를 흩뿌리듯 배치합니다. 일부는 원본 고양이 앞, 일부는 뒤에 배치하여 깊이감(Depth)을 형성합니다. - 핵심 - 행동(Storytelling): 정지된 포즈는 금지입니다. 미니미들은 원본 사진의 배경 사물을 활용하거나, 원본 고양이에게 장난을 치거나, 돕거나, 애정을 표현하는 등 스토리가 느껴지는 역동적인 행동을 해야 합니다. - 미니미들에게도 아주 작고 자연스러운 그림자를 적용해 실제 공간에 존재하는 것처럼 연출합니다. [3. AI 자동 생성 감성 타이틀 & 레이어링 - 중요] - 타이틀 생성 로직: 원본 고양이의 표정과 미니미들의 행동이 만들어내는 '순간의 이야기'를 분석하여, 이를 관통하는 2~5단어의 짧고 감성적인 한글 타이틀을 새로 지으세요. (설명조 금지, 느낌만 전달) - 폰트 및 배치: 부드럽고 자연스러운 손글씨 혹은 타이프라이터 느낌의 폰트를 사용합니다. 텍스트의 일부 글자는 고양이 뒤로, 일부는 앞으로 오게 하여 입체적인 레이어 구조를 만드세요. - 낙서 & 보조 텍스트: 얇은 펜으로 그린 듯한 낙서(미니미의 이동 경로를 나타내는 점선, 미니미의 감정을 나타내는 작은 기호 등 서사와 관련된 것)와 작은 스티커 텍스트(예: mood, little me, sweet moments)를 과하지 않게(3~5개) 추가하여 디테일을 살립니다. [4. 전체 스타일 및 규격] - 필름 감성: 전체적으로 따뜻하고 부드러운 필름 사진 같은 질감을 더합니다. 원본 사진의 색감을 존중하되, 조금 더 포근하고 사랑스러운 분위기로 보정합니다. - 과도한 CG 느낌이나 게임 UI 스타일은 절대 금지합니다. \"잘 꾸민 폴라로이드 사진\" 혹은 \"감성 잡지의 한 페이지\" 같은 자연스러운 합성을 지향합니다. - 이미지 비율: 인스타그램 피드 게재에 가장 최적화된 4:5 세로 비율로 생성해 주세요.",
    "animal": "cat"
  },
  {
    "id": "P-004",
    "title": "우리냥이 관상 분석기",
    "category": "계절",
    "image": "images/p004.jpg",
    "description": "우리아이의 관상을 분석해보세요.",
    "prompt": "Please create a professional and elegant 'cat face physiognomy and charm infographic' based on the attached cat photo, composed in a 4:5 vertical portrait aspect ratio.\n*** NAME & DECORATION: Search your memory and our previous conversations/sessions to see if I have ever mentioned this attached cat's name. If you know the name, naturally incorporate it into the main title (e.g., \"[Name]의 얼굴 관상\") and utilize it in the hand-drawn decorative elements. If you cannot find any record of the name, safely default to '우리냥이' (Our Kitty). DO NOT invent or hallucinate a random name. ***\nA key artistic element is the perspective-correct 'artist's wireframe contour map' of the cat's face. Instead of flat crosshairs, thin white-dotted lines (horizontal and vertical curves) must follow the exact three-dimensional contours, perspective, and form of the cat's specific face to accurately map its features.\nFreely analyze the cat's physical features in the photo. Do not limit the number of phrases or points; use your own judgment to arrange the features that best match the photo. Write positive, touching, and emotionally resonant Korean handwritten-style text, appealing to the warm sensibilities of pet owners. Thin black arrows connect the text to the mapped features.\nAdd delicate and refined hand-drawn decorative elements like elegant pink hearts and subtle sparkles (avoiding an overly childish look). Make excellent use of the tall 4:5 vertical space to arrange text and doodles harmoniously without feeling cluttered.\nA beautiful pink hand-drawn speech bubble at the bottom right contains a final loving conclusion with the cat's name.\nOverall warm, bright, and elegant aesthetic, like a high-end emotional infographic from a premium lifestyle magazine. The subject must be placed on a clean, solid white studio background.\n4:5 VERTICAL PORTRAIT",
    "animal": "cat"
  },
  {
    "id": "P-003",
    "title": "냥다꾸",
    "category": "계절",
    "image": "images/p003.jpg",
    "description": "냥이의 일상을 다꾸",
    "prompt": "[Role] You are a professional creator of Instagram-aesthetic \"Dakku\" (Korean diary decoration) style cat photography. When a user uploads a cat photo, follow these steps to generate a new Dakku-style image. [Workflow] 1. Context & Image Analysis (AI Internal Process): - First, check your memory or previous conversation context to see if the user has mentioned the uploaded cat's 'name'. - Deeply analyze the uploaded photo: identify the cat's specific appearance (fur color, patterns), pose, expression, background, and the overall mood. Interpret the context of the situation. 2. Dakku Theme & Planning (AI Internal Process): - **[Phrase Planning]**: Proactively use the cat's 'name' (if known; otherwise omit) to create cute Korean phrases that naturally fit the mood and composition of the image. Use your creative judgment to determine the appropriate number and placement of phrases to enhance the Dakku aesthetic without cluttering the image. - [Prop Addition Idea]: Devise situation-appropriate, cute virtual props to add as doodle overlays. - [Color Theme Idea]: Determine a 'Personal Color Theme' based on the cat's fur color and overall vibe. - [Aesthetic Decor Idea]: Freely judge and select the most aesthetic scrapbook elements (e.g., ripped paper, masking tape, unique stamps, freeform doodles) that naturally enhance the current image's unique mood. 3. Image Generation (via DALL-E 3): - CRITICAL: The final output image must be a 4:5 vertical portrait aspect ratio, optimal for Instagram feed posts. - Recreate the analyzed cat and background as a 'warm and aesthetic photorealistic image' incorporating the planned theme. - You MUST overlay the following 'Dakku elements' onto the generated image: - All elements must reflect the planned 'Personal Color Theme'. - White hand-drawn doodle outlines naturally tracing the cat's silhouette. - Cute white hand-drawn accents and the planned situation-appropriate hand-drawn props. - The planned Korean phrases placed in a white handwritten font style. - Freely placed, highly aesthetic scrapbook-style decorations based on your creative judgment. [Output Rules] Do not output any long text explaining your analysis or planning process. Directly output the final generated image immediately.",
    "animal": "cat"
  },
  {
    "id": "P-002",
    "title": "고양이 낚시 잡지",
    "category": "귀여운 이미지",
    "image": "images/p002.jpg",
    "description": "이번달 우리 고양이 낚시 잡지를 확인하세요",
    "prompt": "**[Image Reference: User's attached cat photo]** **Objective:** Generate a professional-level, high-resolution magazine cover specifically formatted to a **strict 4:5 vertical portrait aspect ratio** to guarantee perfect, uncropped display in an Instagram feed post. All dynamic content (seasonal theme, fish species, and printed issue date) **MUST** automatically align with the *actual, real-time date of generation*. **Dimensions & Layout (CRITICAL):** * **Aspect Ratio:** This entire image **MUST** be generated in a precise **4:5 vertical portrait ratio** (e.g., 1080px by 1350px). * **Safety Zone:** All text, the cat, and key subjects must be perfectly placed within the central vertical frame to prevent any cropping when uploaded to Instagram. **Subject:** The exact cat from the reference image, maintaining its distinct facial features, fur patterns, and colors perfectly. The cat is styled as an expert angler, wearing a tiny, highly detailed fishing vest and a classic bucket hat. It is proudly posing next to or holding a realistic representation of a \"seasonal fish\" that is in peak season in South Korea *right now* (based on the real-time month). The cat looks directly at the camera with a confident, charismatic presence. **Background:** A stunning fishing environment (e.g., wooden pier, sea boat, or ice fishing site) that perfectly matches the AI-determined season and fish habitat for the *current real-time month*. Include subtle, high-quality fishing gear details. **Style:** Professional magazine cover design, editorial photography, vibrant colors, cinematic lighting, ultra-detailed, 8k resolution. **Text Elements (CRITICAL: ALL TEXT MUST BE WRITTEN IN KOREAN):** * **Magazine Title:** \"월간냥태공\" in massive, bold, eye-catching Korean typography at the very top. Include a small English subtitle \"MONTHLY CATFISHING\" below it. * **Dynamic Headlines:** Generate a main, massive headline and 3 subheadings in Korean that perfectly describe the excitement of fishing the AI-selected seasonal fish for the *current real-time month*. * **Date Details (HIGHEST PRINTING PRIORITY):** The magazine issue date must be clearly rendered. Do **NOT** use random or placeholder years (like 2023 or 2024). You must use the **ACTUAL, CURRENT YEAR** and **ACTUAL, CURRENT MONTH** of generation. Render this exact text clearly in the format: \"[Current Actual Year]년 [Current Actual Month]호\". * **Other Details:** Add realistic elements like a price (e.g., \"8,000원\") and a website URL. **Overall Vibe:** A masterpiece, indistinguishable from a real printed fishing magazine cover in South Korea, completely time-accurate and perfectly optimized for Instagram feeds.",
    "animal": "cat"
  },
  {
    "id": "P-001",
    "title": "[다묘집사] 냥모티콘스티커",
    "category": "사진",
    "image": "images/p001.jpg",
    "description": "내 고양이 이모티콘 생성 프롬프트",
    "prompt": "Create a high-quality 4x4 sheet grid with 16 neat, die-cut sticker-style emojis on a clean white background. The stickers must be strictly and exclusively based on the appearance, features, and individual identity of the specific cat shown in the attached image_0.png. Every single one of the 16 stickers must feature this exact cat, replicating its fur color, pattern, and eye features precisely from the reference image.\nThe AI should choose a collar style and any charms or adornments that it deems most stylish and suitable for this particular cat. The chosen collar design must be applied consistently to all 16 stickers. Each individual sticker has a precise white die-cut border. The entire sheet is a single image.\nCreate 16 unique stickers based on the specific expressions, accompanying icons, and exact Korean text provided below, while maintaining total facial feature and structure consistency with the reference cat. Replicate all icons and Korean text precisely. Row 1 (left to right): Delighted cat licking its mouth. Korean: '최고의 맛이다냥 🐟' Cat winking one eye. Korean: '찡긋! ✨' Cat shocked. Korean: '깜짝이야! 🙀' Cat inquiring. Korean: '지금 바빠? 🐾' Row 2 (left to right): Cat with blissful eyes closed. Korean: '너무 좋아냥~ 🌸' Cat looking forward with loving eyes. Korean: '언제 오냥? 🥺' Cat with a grumpy scowl. Korean: '건드리지 마라냥 💢' Cat laughing with eyes closed. Korean: '신난다냥! 🎶' Row 3 (left to right): Cat with sparkling, starry eyes. Korean: '초롱초롱 👀✨' Cat looking direct. Korean: '내 맘 알지? 💘' Cat offering paws in front. Korean: '안아줘냥 🫂' Cat looking beautiful. Korean: '완벽한 하루냥 👑' Row 4 (left to right): Cat with quizzical head-tilt. Korean: '그게 뭐냥? 🧐' Cat with closed sleeping eyes. Korean: '꿈나라 여행 중 🌙' Cat begging with paws forward. Korean: '간식 주라냥 🍗' Cat wearing the round sunglasses. Korean: '내가 제일 잘나가 😎' The Korean text should be rendered in a cute, hand-drawn font at the bottom of each sticker. The entire sheet must be clean, high-resolution digital art, ready for use as a sticker pack. ar 4:5",
    "animal": "cat"
  }
];
